...
static int check_solution(int, int , MORSE_Complex32_t *, int, MORSE_Complex32_t *, MORSE_Complex32_t *, int, float);
...
int testing_cgesv_incpiv(int argc, char **argv)
{
    ...
    MORSE_Complex32_t *A1 = (MORSE_Complex32_t *)malloc(LDA*N*(sizeof*A1));
    MORSE_Complex32_t *A2 = (MORSE_Complex32_t *)malloc(LDA*N*(sizeof*A2));
    MORSE_Complex32_t *B1 = (MORSE_Complex32_t *)malloc(LDB*NRHS*(sizeof*B1));
    MORSE_Complex32_t *B2 = (MORSE_Complex32_t *)malloc(LDB*NRHS*(sizeof*B2));
    MORSE_desc_t *L;
    int *IPIV;
    ...
    */
    ...
    /* MORSE CGESV */
    MORSE_Alloc_Workspace_cgesv_incpiv(N, &L, &IPIV);
    MORSE_cgesv_incpiv(N, NRHS, A2, LDA, L, IPIV, B2, LDB);

    printf("\n");
    printf("------ TESTS FOR CHAMELEON INCPIV CGESV ROUTINE -------  \n");
    ...
    */
    ...
    /* Morse routines */
    MORSE_cgetrf_incpiv(N, N, A2, LDA, L, IPIV);
    MORSE_cgetrs_incpiv(MorseNoTrans, N, NRHS, A2, LDA, L, IPIV, B2, LDB);

    printf("\n");
    printf("------ TESTS FOR CHAMELEON CGETRF + CGETRS ROUTINE -------  \n");
    printf("            Size of the Matrix %d by %d\n", N, N);
    printf("\n");
    printf(" The matrix A is randomly generated for each test.\n");
    printf("============\n");
    ...
    */
    ...
    /* MORSE routines */
    MORSE_cgetrf_incpiv(N, N, A2, LDA, L, IPIV);
    MORSE_ctrsmpl(N, NRHS, A2, LDA, L, IPIV, B2, LDB);
    MORSE_ctrsm(MorseLeft, MorseUpper, MorseNoTrans, MorseNonUnit,
                 N, NRHS, 1.0, A2, LDA, B2, LDB);

    printf("\n");
    printf("------ TESTS FOR CHAMELEON INCPIV CGETRF + CTRSMPL + CTRSM  ROUTINE -------  \n");
    printf("            Size of the Matrix %d by %d\n", N, N);
    printf("\n");
    printf(" The matrix A is randomly generated for each test.\n");
    printf("============\n");
...
static int check_solution(int N, int NRHS, MORSE_Complex32_t *A1, int LDA, MORSE_Complex32_t *B1, MORSE_Complex32_t *B2, int LDB, float eps )
{
    int info_solution;
