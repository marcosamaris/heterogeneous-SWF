...
int testing_cgemm(int argc, char **argv)
{
    ...
    if (MORSE_My_Mpi_Rank() == 0){
        printf("\n");
        printf("------ TESTS FOR CHAMELEON CGEMM ROUTINE -------  \n");
    ...
    for (ta=0; ta<2; ta++) {
        for (tb=0; tb<2; tb++) {
            ...
            /* MORSE CGEMM */
            MORSE_cgemm(trans[ta], trans[tb], M, N, K, alpha, A, LDA, B, LDB, beta, Cfinal, LDC);
                          ...
                          MORSE_Complex32_t beta, MORSE_Complex32_t *Cref, MORSE_Complex32_t *Cmorse, int LDC)
{
    ...
    Anorm       = LAPACKE_clange_work(LAPACK_COL_MAJOR, morse_lapack_const(MorseInfNorm), Am, An, A,       LDA, work);
    Bnorm       = LAPACKE_clange_work(LAPACK_COL_MAJOR, morse_lapack_const(MorseInfNorm), Bm, Bn, B,       LDB, work);
