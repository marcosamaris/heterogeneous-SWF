...
int testing_dlange(int argc, char **argv)
{
    ...
    printf("\n");
    printf("------ TESTS FOR CHAMELEON DLANGE ROUTINE -------  \n");
    ...
    /* Initialize A, B, C */
    MORSE_dplrnt( M, N, A, LDA, 3436 );

    /* MORSE DLANGE */
    for(n=0; n<4; n++) {
        normmorse  = MORSE_dlange(norm[n], M, N, A, LDA);
        normlapack = LAPACKE_dlange_work(LAPACK_COL_MAJOR, morse_lapack_const(norm[n]), M, N, A, LDA, work);
        ...
        for(n=1; n<3; n++) {
            for(u=0; u<2; u++) {
                int d;
                for(d=0; d<2; d++) {
                    normmorse = MORSE_dlantr(norm[n], uplo[u], diag[d], M, N, A, LDA);
                    normlapack = LAPACKE_dlantr_work(LAPACK_COL_MAJOR, morse_lapack_const(norm[n]), morse_lapack_const(uplo[u]),
    ...
    for(n=0; n<4; n++) {
        for(u=0; u<2; u++) {
            normmorse = MORSE_dlansy(norm[n], uplo[u], min(M,N), A, LDA);
            normlapack = LAPACKE_dlansy_work(LAPACK_COL_MAJOR, morse_lapack_const(norm[n]), morse_lapack_const(uplo[u]), min(M,N), A, LDA, work);
            printf("LAPACK %e, CHAMELEON %e\n", normlapack, normmorse);
