...
{
    (void)nb;
    struct starpu_codelet *codelet = &cl_stsqrt;
    void (*callback)(void*) = options->profiling ? cl_stsqrt_callback : NULL;
...
static void cl_stsqrt_cpu_func(void *descr[], void *cl_arg)
{
    ...
    T  = (float *)STARPU_MATRIX_GET_PTR(descr[2]);
    TAU= (float *)STARPU_MATRIX_GET_PTR(descr[3]); /* nb + ib*nb */
...
magma_int_t
magma_stsqrt2_gpu( magma_int_t m, magma_int_t n, magma_int_t nb,
...
//	hwork[0] = *((float*) &lwkopt);
	...
	memset(t, 0, nb*nb*sizeof(float));
	cudaMemset(dt, 0, nb*n*sizeof(float));

	/* copy the first diag tile of A1 from device to host: da1 -> d */
	cublasGetMatrix(nb, nb, sizeof(float),
                    da1_ref(0, 0), ldda1,
                    d, ldd);
...
//	cublasGetMatrix(m, nb, sizeof(float),
//                    da2_ref(0, 0), ldda2,
//                    a2, lda2);
...
//                rows*ib*sizeof(float),
//                cudaMemcpyDeviceToHost );

			/* Apply H' to A(i:m,i+2*ib:n) from the left */
			cols = n-old_i-2*old_ib;
			if (cols > 0){
				magma_stsmqr_gpu( MagmaLeft, MagmaTrans,
...
static void cl_stsqrt_cuda_func(void *descr[], void *cl_arg)
{
    MORSE_starpu_ws_t *h_work;
    int m;
    int n;
    int ib;
    float *h_A2, *h_T, *h_D, *h_TAU, *h_W;
    float *d_A1, *d_A2, *d_T, *d_D, *d_W;
    int lda1, lda2, ldt;
    CUstream stream;
    ...
    d_A1 = (float *)STARPU_MATRIX_GET_PTR(descr[0]);
    d_A2 = (float *)STARPU_MATRIX_GET_PTR(descr[1]);
    d_T  = (float *)STARPU_MATRIX_GET_PTR(descr[2]);
    d_W  = (float *)STARPU_MATRIX_GET_PTR(descr[3]); /* 2*ib*n + ib*ib */
    d_D  = d_W + 2*ib*n;

    /* scratch data on host */
    /* m*ib + ib*ib + max(m,n) + ib*n + ib*ib */
    h_A2  = (float*)RUNTIME_starpu_ws_getlocal(h_work);
    h_T   = h_A2  + m*ib;
    h_TAU = h_T   + ib*ib;
    h_W   = h_TAU + max(m,n);
    h_D   = h_W   + ib*n;
...
CODELETS(stsqrt, 4, cl_stsqrt_cpu_func, cl_stsqrt_cuda_func, 0)
#else
CODELETS_CPU(stsqrt, 4, cl_stsqrt_cpu_func)
#endif
