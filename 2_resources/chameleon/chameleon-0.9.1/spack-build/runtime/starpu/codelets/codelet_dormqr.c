 ...
 *
 *  CORE_dormqr overwrites the general real M-by-N tile C with
 *
 *                    SIDE = 'L'     SIDE = 'R'
 *    TRANS = 'N':      Q * C          C * Q
 *    TRANS = 'C':      Q**T * C       C * Q**T
...
{
    struct starpu_codelet *codelet = &cl_dormqr;
    void (*callback)(void*) = options->profiling ? cl_dormqr_callback : NULL;
...
static void cl_dormqr_cpu_func(void *descr[], void *cl_arg)
{
    MORSE_enum side;
    MORSE_enum trans;
    int m;
...
magma_int_t
magma_dormqrt_gpu( magma_side_t side, magma_trans_t trans,
...
static void cl_dormqr_cuda_func(void *descr[], void *cl_arg)
{
    ...
    magma_dormqrt_gpu( side, trans, m, n, k, ib,
                       A, lda, T, ldt, C, ldc, WORK, ldwork );
...
CODELETS(dormqr, 4, cl_dormqr_cpu_func, cl_dormqr_cuda_func, 0)
#else
CODELETS_CPU(dormqr, 4, cl_dormqr_cpu_func)
#endif
