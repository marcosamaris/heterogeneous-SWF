 ...
 *
 *  CORE_sssssm applies the LU factorization update from a real
...
{
    (void)nb;
    struct starpu_codelet *codelet = &cl_sssssm;
    void (*callback)(void*) = options->profiling ? cl_sssssm_callback : NULL;
         ...
         morse_desc_islocal( L2, L2m, L2n ) )
    {
        starpu_insert_task(
            ...
            STARPU_R,             RTBLKADDR(L1, float, L1m, L1n),
            STARPU_VALUE,  &ldl1,                        sizeof(int),
            STARPU_R,             RTBLKADDR(L2, float, L2m, L2n),
...
static void cl_sssssm_cpu_func(void *descr[], void *cl_arg)
{
    int m1;
    int n1;
    int m2;
    ...
    L1 = (float *)STARPU_MATRIX_GET_PTR(descr[2]);
    L2 = (float *)STARPU_MATRIX_GET_PTR(descr[3]);
...
static void cl_sssssm_cuda_func(void *descr[], void *cl_arg)
{
    int m1;
    int n1;
    int m2;
    ...
    dA1  = (float *)STARPU_MATRIX_GET_PTR(descr[0]);
    dA2  = (float *)STARPU_MATRIX_GET_PTR(descr[1]);
    dL1  = (float *)STARPU_MATRIX_GET_PTR(descr[2]);
    dL2  = (float *)STARPU_MATRIX_GET_PTR(descr[3]);
...
CODELETS(sssssm, 4, cl_sssssm_cpu_func, cl_sssssm_cuda_func, 0)
#else
CODELETS_CPU(sssssm, 4, cl_sssssm_cpu_func)
#endif
