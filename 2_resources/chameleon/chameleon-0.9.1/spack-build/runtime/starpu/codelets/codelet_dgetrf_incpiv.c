...
{
    (void)nb;
    struct starpu_codelet *codelet = &cl_dgetrf_incpiv;
    void (*callback)(void*) = options->profiling ? cl_dgetrf_incpiv_callback : NULL;
...
static void cl_dgetrf_incpiv_cpu_func(void *descr[], void *cl_arg)
{
    ...
    L = (double *)STARPU_MATRIX_GET_PTR(descr[1]);
	...
	L += ib;
        for (i=0; i<n; i+=ib) {
            sb = min( ib, n-i );
            CORE_dlacpy(MorseUpperLower, sb, sb, A+(i*lda+i), lda, L+(i*ldl), ldl );

            CORE_dtrtri( MorseLower, MorseUnit, sb, L+(i*ldl), ldl, &info );
            if (info != 0 ) {
...
static void cl_dgetrf_incpiv_cuda_func(void *descr[], void *cl_arg)
{
    int m;
    int n;
    int ib;
    ...
    dA = (double *)STARPU_MATRIX_GET_PTR(descr[0]);
    dL = (double *)STARPU_MATRIX_GET_PTR(descr[1]);
    /*
     * hwork => at least (IB+NB)*IB contains all hA and hL
     * dwork => at least IB*NB
     */
    hA    = (double*)RUNTIME_starpu_ws_getlocal(h_work);
    dwork = (double *)STARPU_MATRIX_GET_PTR(descr[2]);

    hL = hA + lda*ib;

    /* Initialize L to 0 */
    memset(hL, 0, ib*ib*sizeof(double));
    ...
    magma_dgetrf_incpiv_gpu( MagmaColMajor, m, n, ib,
                             hA, lda, dA, lda,
                             hL, ib,  dL, ldl,
                             IPIV,
                             dwork, lda,
...
CODELETS(dgetrf_incpiv, 3, cl_dgetrf_incpiv_cpu_func, cl_dgetrf_incpiv_cuda_func, 0)
#else
CODELETS_CPU(dgetrf_incpiv, 3, cl_dgetrf_incpiv_cpu_func)
#endif
