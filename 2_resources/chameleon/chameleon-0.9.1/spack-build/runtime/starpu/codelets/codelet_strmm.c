...
{
    (void)nb;
    struct starpu_codelet *codelet = &cl_strmm;
    void (*callback)(void*) = options->profiling ? cl_strmm_callback : NULL;
...
static void cl_strmm_cpu_func(void *descr[], void *cl_arg)
{
    MORSE_enum side;
    MORSE_enum uplo;
    MORSE_enum transA;
...
static void cl_strmm_cuda_func(void *descr[], void *cl_arg)
{
    MORSE_enum side;
    MORSE_enum uplo;
    MORSE_enum transA;
    ...
    if (side == MorseLeft){
        ...
        fprintf(stderr, "Error in cl_strmm_cuda_func: bad side parameter %d\n", side);
    }
    if (uplo == MorseUpper){
        ...
        fprintf(stderr, "Error in cl_strmm_cuda_func: bad uplo parameter %d\n", uplo);
    }
    if (transA == MorseNoTrans){
        ...
        fprintf(stderr, "Error in cl_strmm_cuda_func: bad transA parameter %d\n", transA);
    }
    if (diag == MorseNonUnit){
        ...
        fprintf(stderr, "Error in cl_strmm_cuda_func: bad diag parameter %d\n", diag);
    }

    stat = cublasStrmm( handle,
        cublasSide, cublasUplo, cublasTransA, cublasDiag,
        M, N,
        (const float *) &alpha, A, LDA,
        B, LDB, B, LDB);
    if (stat != CUBLAS_STATUS_SUCCESS){
        printf ("cublasStrmm failed");
 ...
 * Codelet definition
 */
CODELETS(strmm, 2, cl_strmm_cpu_func, cl_strmm_cuda_func, STARPU_CUDA_ASYNC)
