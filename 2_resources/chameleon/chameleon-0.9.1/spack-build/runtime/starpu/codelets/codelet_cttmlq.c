...
{
    (void)nb;
    struct starpu_codelet *codelet = &cl_cttmlq;
    void (*callback)(void*) = options->profiling ? cl_cttmlq_callback : NULL;
...
static void cl_cttmlq_cpu_func(void *descr[], void *cl_arg)
{
    MORSE_enum side;
    MORSE_enum trans;
    int m1;
    ...
    WORK = (MORSE_Complex32_t *)STARPU_MATRIX_GET_PTR(descr[4]); /* nb * ib */
 ...
 * Codelet definition
 */
CODELETS_CPU(cttmlq, 5, cl_cttmlq_cpu_func)
