...
{
    struct starpu_codelet *codelet = &cl_clanhe;
...
static void cl_clanhe_cpu_func(void *descr[], void *cl_arg)
{
    float *normA;
    MORSE_enum norm;
    MORSE_enum uplo;
    ...
    CORE_clanhe( norm, uplo, N, A, LDA, work, normA);
}

/*
 * Codelet definition
 */
CODELETS_CPU(clanhe, 3, cl_clanhe_cpu_func)
