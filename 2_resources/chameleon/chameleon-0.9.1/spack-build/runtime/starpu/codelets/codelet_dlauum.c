...
{
    (void)nb;
    struct starpu_codelet *codelet = &cl_dlauum;
    void (*callback)(void*) = options->profiling ? cl_dlauum_callback : NULL;
...
static void cl_dlauum_cpu_func(void *descr[], void *cl_arg)
{
    MORSE_enum uplo;
    int N;
    double *A;
...
static void cl_dlauum_cuda_func(void *descr[], void *cl_arg)
{
    MORSE_enum uplo;
    int ret;
    int info = 0;
...
CODELETS(dlauum, 1, cl_dlauum_cpu_func, cl_dlauum_cuda_func, 0)
#else
CODELETS_CPU(dlauum, 1, cl_dlauum_cpu_func)
#endif
