...
{
    struct starpu_codelet *codelet = &cl_dtrssq;
...
static void cl_dtrssq_cpu_func(void *descr[], void *cl_arg)
{
    MORSE_enum uplo;
    MORSE_enum diag;
    int m;
    ...
    CORE_dtrssq( uplo, diag, m, n, A, lda, &SCALESUMSQ[0], &SCALESUMSQ[1]);
}
/*
 * Codelet definition
 */
CODELETS_CPU(dtrssq, 2, cl_dtrssq_cpu_func)
