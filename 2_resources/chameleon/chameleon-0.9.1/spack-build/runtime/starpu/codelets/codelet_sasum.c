...
void MORSE_TASK_sasum(MORSE_option_t *options,
                       MORSE_enum storev, MORSE_enum uplo, int M, int N,
                       MORSE_desc_t *A, int Am, int An, int lda,
                       MORSE_desc_t *B, int Bm, int Bn)
{
    struct starpu_codelet *codelet = &cl_sasum;
    void (*callback)(void*) = options->profiling ? cl_sasum_callback : NULL;
    starpu_insert_task(
        codelet,
...
static void cl_sasum_cpu_func(void *descr[], void *cl_arg)
{
    MORSE_enum storev;
    MORSE_enum uplo;
    int M;
 ...
 * Codelet definition
 */
CODELETS_CPU(sasum, 2, cl_sasum_cpu_func)
