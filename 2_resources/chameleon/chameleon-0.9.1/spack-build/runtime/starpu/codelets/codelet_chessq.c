...
{
    struct starpu_codelet *codelet = &cl_chessq;
...
static void cl_chessq_cpu_func(void *descr[], void *cl_arg)
{
    MORSE_enum uplo;
    int n;
    MORSE_Complex32_t *A;
    ...
    CORE_chessq( uplo, n, A, lda, &SCALESUMSQ[0], &SCALESUMSQ[1] );
}
/*
 * Codelet definition
 */
CODELETS_CPU(chessq, 2, cl_chessq_cpu_func)
