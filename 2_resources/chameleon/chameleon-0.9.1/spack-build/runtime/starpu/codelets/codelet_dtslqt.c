...
{
    (void)nb;
    struct starpu_codelet *codelet = &cl_dtslqt;
    void (*callback)(void*) = options->profiling ? cl_dtslqt_callback : NULL;
...
static void cl_dtslqt_cpu_func(void *descr[], void *cl_arg)
{
    ...
    TAU= (double *)STARPU_MATRIX_GET_PTR(descr[3]); /* nb + ib*nb */
...
magma_int_t
magma_dtslqt_gpu( magma_int_t m, magma_int_t n, magma_int_t nb,
...
#define d_ref(a_1,a_2) ( d+(a_2)*ldd + (a_1))
    ...
    for (i = 0; i < m; i += nb) {
        ...
        if (i>0) {
            ...
            cublasGetMatrix(ib, cols, sizeof(double),
                            da2_ref(i, 0), ldda2,
                            a2, lda2);
            ...
            if (rows > 0){
                magma_dtsmlq_gpu( MagmaRight, MagmaTrans,
...
static void cl_dtslqt_cuda_func(void *descr[], void *cl_arg)
{
    MORSE_starpu_ws_t *h_work;
    int m;
    int n;
    int ib;
    double *h_A2, *h_T, *h_D, *h_TAU, *h_W;
    double *d_A1, *d_A2, *d_T, *d_D, *d_W;
    int lda1, lda2, ldt;
    CUstream stream;
    ...
    d_A1 = (double *)STARPU_MATRIX_GET_PTR(descr[0]);
    d_A2 = (double *)STARPU_MATRIX_GET_PTR(descr[1]);
    d_T  = (double *)STARPU_MATRIX_GET_PTR(descr[2]);
    d_W  = (double *)STARPU_MATRIX_GET_PTR(descr[3]); /* 2*ib*m + ib*ib */
    d_D  = d_W + 2*ib*m;

    /* scratch data on host */
    /* ib*n + ib*n + max(m,n) + ib*m + ib*ib */
    h_A2  = (double*)RUNTIME_starpu_ws_getlocal(h_work);
    h_T   = h_A2  + ib*n;
    h_TAU = h_T   + ib*n;
    h_W   = h_TAU + max(m,n);
    h_D   = h_W   + ib*m;
...
CODELETS(dtslqt, 4, cl_dtslqt_cpu_func, cl_dtslqt_cuda_func, 0)
#else
CODELETS_CPU(dtslqt, 4, cl_dtslqt_cpu_func)
#endif
