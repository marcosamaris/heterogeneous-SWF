 ...
 *
 *  CORE_dssssm applies the LU factorization update from a real
...
{
    (void)nb;
    struct starpu_codelet *codelet = &cl_dssssm;
    void (*callback)(void*) = options->profiling ? cl_dssssm_callback : NULL;
         ...
         morse_desc_islocal( L2, L2m, L2n ) )
    {
        starpu_insert_task(
            ...
            STARPU_R,             RTBLKADDR(L1, double, L1m, L1n),
            STARPU_VALUE,  &ldl1,                        sizeof(int),
            STARPU_R,             RTBLKADDR(L2, double, L2m, L2n),
...
static void cl_dssssm_cpu_func(void *descr[], void *cl_arg)
{
    int m1;
    int n1;
    int m2;
    ...
    L1 = (double *)STARPU_MATRIX_GET_PTR(descr[2]);
    L2 = (double *)STARPU_MATRIX_GET_PTR(descr[3]);
...
static void cl_dssssm_cuda_func(void *descr[], void *cl_arg)
{
    int m1;
    int n1;
    int m2;
    ...
    dA1  = (double *)STARPU_MATRIX_GET_PTR(descr[0]);
    dA2  = (double *)STARPU_MATRIX_GET_PTR(descr[1]);
    dL1  = (double *)STARPU_MATRIX_GET_PTR(descr[2]);
    dL2  = (double *)STARPU_MATRIX_GET_PTR(descr[3]);
...
CODELETS(dssssm, 4, cl_dssssm_cpu_func, cl_dssssm_cuda_func, 0)
#else
CODELETS_CPU(dssssm, 4, cl_dssssm_cpu_func)
#endif
