...
static void cl_strsm_cuda_func(void *descr[], void *cl_arg)
{
    ...
    if (side == MorseLeft){
        ...
        fprintf(stderr, "Error in cl_strsm_cuda_func: bad side parameter %d\n", side);
    }
    if (uplo == MorseUpper){
        ...
        fprintf(stderr, "Error in cl_strsm_cuda_func: bad uplo parameter %d\n", uplo);
    }
    if (transA == MorseNoTrans){
        ...
        fprintf(stderr, "Error in cl_strsm_cuda_func: bad transA parameter %d\n", transA);
    }
    if (diag == MorseNonUnit){
        ...
        fprintf(stderr, "Error in cl_strsm_cuda_func: bad diag parameter %d\n", diag);
    }
 ...
 * Codelet definition
 */
CODELETS(strsm, 2, cl_strsm_cpu_func, cl_strsm_cuda_func, STARPU_CUDA_ASYNC)
