...
{
    (void)nb;
    struct starpu_codelet *codelet = &cl_cttqrt;
    void (*callback)(void*) = options->profiling ? cl_cttqrt_callback : NULL;
...
static void cl_cttqrt_cpu_func(void *descr[], void *cl_arg)
{
    int m;
    int n;
    int ib;
    ...
    TAU = (MORSE_Complex32_t *)STARPU_MATRIX_GET_PTR(descr[3]); /* nb * (ib+1) */
 ...
 * Codelet definition
 */
CODELETS_CPU(cttqrt, 4, cl_cttqrt_cpu_func)
