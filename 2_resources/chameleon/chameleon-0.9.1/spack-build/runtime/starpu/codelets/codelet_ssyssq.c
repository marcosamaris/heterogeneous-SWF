...
{
    struct starpu_codelet *codelet = &cl_ssyssq;
...
static void cl_ssyssq_cpu_func(void *descr[], void *cl_arg)
{
    MORSE_enum uplo;
    int n;
    float *A;
    ...
    CORE_ssyssq( uplo, n, A, lda, &SCALESUMSQ[0], &SCALESUMSQ[1] );
}
/*
 * Codelet definition
 */
CODELETS_CPU(ssyssq, 2, cl_ssyssq_cpu_func)
