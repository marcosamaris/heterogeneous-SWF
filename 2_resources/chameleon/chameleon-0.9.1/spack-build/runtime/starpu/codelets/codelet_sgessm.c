...
{
    (void)nb;
    struct starpu_codelet *codelet = &cl_sgessm;
    void (*callback)(void*) = options->profiling ? cl_sgessm_callback : NULL;
         ...
         morse_desc_islocal( A, Am, An ) )
    {
        starpu_insert_task(
            ...
            STARPU_R,             RTBLKADDR(L, float, Lm, Ln),
            STARPU_VALUE,   &ldl,                        sizeof(int),
            STARPU_R,             RTBLKADDR(D, float, Dm, Dn),
...
static void cl_sgessm_cpu_func(void *descr[], void *cl_arg)
{
    int m;
    int n;
    int k;
    ...
    L = (float *)STARPU_MATRIX_GET_PTR(descr[0]);
    D = (float *)STARPU_MATRIX_GET_PTR(descr[1]);
    A = (float *)STARPU_MATRIX_GET_PTR(descr[2]);
    starpu_codelet_unpack_args(cl_arg, &m, &n, &k, &ib, &IPIV, &ldl, &ldd, &lda);
    CORE_sgessm(m, n, k, ib, IPIV, D, ldd, A, lda);
}

#if defined(CHAMELEON_USE_MAGMA) && defined(HAVE_MAGMA_GETRF_INCPIV_GPU)
static void cl_sgessm_cuda_func(void *descr[], void *cl_arg)
{
    int m;
    int n;
    int k;
     ...
     *  hwork => nb*nb
     */
    dL = (float *)STARPU_MATRIX_GET_PTR(descr[0]);
    dD = (float *)STARPU_MATRIX_GET_PTR(descr[1]);
    dA = (float *)STARPU_MATRIX_GET_PTR(descr[2]);
    starpu_codelet_unpack_args(cl_arg, &m, &n, &k, &ib, &IPIV, &lddl, &lddd, &ldda);

    /* The kernel is just using the inverted part or nothing */
    if ( lddl >= 2*ib ) {
      dL += ib;
      ret = magma_sgessm_gpu( MagmaColMajor, m, n, k, ib,
...
CODELETS(sgessm, 3, cl_sgessm_cpu_func, cl_sgessm_cuda_func, 0)
#else
CODELETS_CPU(sgessm, 3, cl_sgessm_cpu_func)
#endif
