...
{
    (void)nb;
    struct starpu_codelet *codelet = &cl_ctrtri;
    void (*callback)(void*) = options->profiling ? cl_ctrtri_callback : NULL;
...
static void cl_ctrtri_cpu_func(void *descr[], void *cl_arg)
{
    MORSE_enum uplo;
    MORSE_enum diag;
    int N;
...
static void cl_ctrtri_cuda_func(void *descr[], void *cl_arg)
{
    MORSE_enum uplo;
    MORSE_enum diag;
    int N;
    ...
    starpu_codelet_unpack_args(cl_arg, &uplo, &diag, &N, &LDA, &iinfo);
    ret = magma_ctrtri_gpu( uplo, diag,
			    N, A, LDA, &info);
...
CODELETS(ctrtri, 1, cl_ctrtri_cpu_func, cl_ctrtri_cuda_func, 0)
#else
CODELETS_CPU(ctrtri, 1, cl_ctrtri_cpu_func)
#endif
