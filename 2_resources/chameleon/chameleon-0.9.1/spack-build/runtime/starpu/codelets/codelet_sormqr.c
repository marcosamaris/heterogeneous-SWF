 ...
 *
 *  CORE_sormqr overwrites the general real M-by-N tile C with
 *
 *                    SIDE = 'L'     SIDE = 'R'
 *    TRANS = 'N':      Q * C          C * Q
 *    TRANS = 'C':      Q**T * C       C * Q**T
...
{
    struct starpu_codelet *codelet = &cl_sormqr;
    void (*callback)(void*) = options->profiling ? cl_sormqr_callback : NULL;
...
static void cl_sormqr_cpu_func(void *descr[], void *cl_arg)
{
    MORSE_enum side;
    MORSE_enum trans;
    int m;
...
magma_int_t
magma_sormqrt_gpu( magma_side_t side, magma_trans_t trans,
...
static void cl_sormqr_cuda_func(void *descr[], void *cl_arg)
{
    ...
    magma_sormqrt_gpu( side, trans, m, n, k, ib,
                       A, lda, T, ldt, C, ldc, WORK, ldwork );
...
CODELETS(sormqr, 4, cl_sormqr_cpu_func, cl_sormqr_cuda_func, 0)
#else
CODELETS_CPU(sormqr, 4, cl_sormqr_cpu_func)
#endif
