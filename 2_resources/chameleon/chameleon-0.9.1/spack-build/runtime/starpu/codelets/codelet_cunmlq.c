...
{
    struct starpu_codelet *codelet = &cl_cunmlq;
    void (*callback)(void*) = options->profiling ? cl_cunmlq_callback : NULL;
...
static void cl_cunmlq_cpu_func(void *descr[], void *cl_arg)
{
    MORSE_enum side;
    MORSE_enum trans;
    int m;
    ...
    WORK = (MORSE_Complex32_t *)STARPU_MATRIX_GET_PTR(descr[3]); /* ib * nb */
...
magma_int_t
magma_cunmlqt_gpu( magma_side_t side, magma_trans_t trans,
                   ...
                   magmaFloatComplex *WORK, magma_int_t LDWORK )
{
    ...
    for(i = i1; (i >- 1) && (i < K); i+=i3 ) {
        ...
        magma_clarfb_gpu( side, trans, MagmaForward, MagmaRowwise,
                          mi, ni, kb,
                          A + LDA * i  + i,  LDA,
                          T + LDT * i,       LDT,
                          C + LDC * jc + ic, LDC,
...
static void cl_cunmlq_cuda_func(void *descr[], void *cl_arg)
{
    ...
    T    = (cuFloatComplex *)STARPU_MATRIX_GET_PTR(descr[1]);
    C    = (cuFloatComplex *)STARPU_MATRIX_GET_PTR(descr[2]);
    WORK = (cuFloatComplex *)STARPU_MATRIX_GET_PTR(descr[3]); /* ib * nb */

    magma_cunmlqt_gpu( side, trans, m, n, k, ib,
                       A, lda, T, ldt, C, ldc, WORK, ldwork );
...
CODELETS(cunmlq, 4, cl_cunmlq_cpu_func, cl_cunmlq_cuda_func, 0)
#else
CODELETS_CPU(cunmlq, 4, cl_cunmlq_cpu_func)
#endif
