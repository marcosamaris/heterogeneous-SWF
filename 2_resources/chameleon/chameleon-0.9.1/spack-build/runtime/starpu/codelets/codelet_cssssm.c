...
{
    (void)nb;
    struct starpu_codelet *codelet = &cl_cssssm;
    void (*callback)(void*) = options->profiling ? cl_cssssm_callback : NULL;
         ...
         morse_desc_islocal( L2, L2m, L2n ) )
    {
        starpu_insert_task(
            ...
            STARPU_R,             RTBLKADDR(L1, MORSE_Complex32_t, L1m, L1n),
            STARPU_VALUE,  &ldl1,                        sizeof(int),
            STARPU_R,             RTBLKADDR(L2, MORSE_Complex32_t, L2m, L2n),
...
static void cl_cssssm_cpu_func(void *descr[], void *cl_arg)
{
    int m1;
    int n1;
    int m2;
    ...
    int lda2;
    MORSE_Complex32_t *L1;
    int ldl1;
    MORSE_Complex32_t *L2;
    int ldl2;
    int *IPIV;

    A1 = (MORSE_Complex32_t *)STARPU_MATRIX_GET_PTR(descr[0]);
    A2 = (MORSE_Complex32_t *)STARPU_MATRIX_GET_PTR(descr[1]);
    L1 = (MORSE_Complex32_t *)STARPU_MATRIX_GET_PTR(descr[2]);
    L2 = (MORSE_Complex32_t *)STARPU_MATRIX_GET_PTR(descr[3]);
...
static void cl_cssssm_cuda_func(void *descr[], void *cl_arg)
{
    int m1;
    int n1;
    int m2;
    ...
    dA1  = (cuFloatComplex *)STARPU_MATRIX_GET_PTR(descr[0]);
    dA2  = (cuFloatComplex *)STARPU_MATRIX_GET_PTR(descr[1]);
    dL1  = (cuFloatComplex *)STARPU_MATRIX_GET_PTR(descr[2]);
    dL2  = (cuFloatComplex *)STARPU_MATRIX_GET_PTR(descr[3]);
...
CODELETS(cssssm, 4, cl_cssssm_cpu_func, cl_cssssm_cuda_func, 0)
#else
CODELETS_CPU(cssssm, 4, cl_cssssm_cpu_func)
#endif
