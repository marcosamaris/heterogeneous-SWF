...
{
    (void)nb;
    struct starpu_codelet *codelet = &cl_sgetrf;
    void (*callback)(void*) = options->profiling ? cl_sgetrf_callback : NULL;
...
static void cl_sgetrf_cpu_func(void *descr[], void *cl_arg)
{
    int m;
    int n;
    float *A;
 ...
 * Codelet definition
 */
CODELETS_CPU(sgetrf, 1, cl_sgetrf_cpu_func)
