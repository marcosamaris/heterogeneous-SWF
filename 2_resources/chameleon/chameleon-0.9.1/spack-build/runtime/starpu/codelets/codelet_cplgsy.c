                        ...
                        int bigM, int m0, int n0, unsigned long long int seed )
{
    ...
    struct starpu_codelet *codelet = &cl_cplgsy;
    void (*callback)(void*) = options->profiling ? cl_cplgsy_callback : NULL;

    if ( morse_desc_islocal( A, Am, An ) )
    {
        starpu_insert_task(
            codelet,
            STARPU_VALUE, &bump,       sizeof(MORSE_Complex32_t),
...
static void cl_cplgsy_cpu_func(void *descr[], void *cl_arg)
{
    MORSE_Complex32_t bump;
    int m;
    int n;
    MORSE_Complex32_t *A;
    int lda;
 ...
 * Codelet definition
 */
CODELETS_CPU(cplgsy, 1, cl_cplgsy_cpu_func)
