 ...
 *
 *  CORE_ststrf computes an LU factorization of a real matrix formed
...
{
    (void)nb;
    struct starpu_codelet *codelet = &cl_ststrf;
    void (*callback)(void*) = options->profiling ? cl_ststrf_callback : NULL;
         ...
         morse_desc_islocal( L, Lm, Ln ) )
    {
        starpu_insert_task(
            ...
            STARPU_RW,        RTBLKADDR(U, float, Um, Un),
            STARPU_VALUE,    &ldu,                       sizeof(int),
            STARPU_RW,        RTBLKADDR(A, float, Am, An),
            STARPU_VALUE,    &lda,                       sizeof(int),
            STARPU_W,         RTBLKADDR(L, float, Lm, Ln),
...
static void cl_ststrf_cpu_func(void *descr[], void *cl_arg)
{
    ...
    U = (float *)STARPU_MATRIX_GET_PTR(descr[0]);
    A = (float *)STARPU_MATRIX_GET_PTR(descr[1]);
    L = (float *)STARPU_MATRIX_GET_PTR(descr[2]);
    WORK = (float *)STARPU_MATRIX_GET_PTR(descr[3]);
    ...
    if ( ldl >= 2*ib )
    {
        ...
        for (i=0; i<n; i+=ib) {
            sb = min( ib, n-i );
            CORE_slacpy(MorseUpperLower, sb, sb, L+(i*ldl), ldl, L+(i*ldl)+ib, ldl );

            CORE_strtri( MorseLower, MorseUnit, sb, L+(i*ldl)+ib, ldl, &info );
            if (info != 0 ) {
...
static void cl_ststrf_cuda_func(void *descr[], void *cl_arg)
{
    MORSE_starpu_ws_t *d_work;
    int m;
    int n;
    ...
    dU = (float *)STARPU_MATRIX_GET_PTR(descr[0]);
    dA = (float *)STARPU_MATRIX_GET_PTR(descr[1]);
    dL = (float *)STARPU_MATRIX_GET_PTR(descr[2]);

    /*
     *  hwork => 2*nb*(2*ib+2nb)
     *  dwork => 2*ib*nb
     */
    hw2 = (float *)STARPU_MATRIX_GET_PTR(descr[3]);
    dw  = (float*)RUNTIME_starpu_ws_getlocal(d_work);
    ...
    cublasGetMatrix( nb, n,  sizeof(float), dU, ldu, hU, ldu );
    cublasGetMatrix( m,  ib, sizeof(float), dA, lda, hA, lda );

    /* Initialize L to 0 */
    memset(hL, 0, ldl*nb*sizeof(float));

    magma_ststrf_gpu( MagmaColMajor, m, n, ib, nb,
                      hU, ldu, dU, ldu,
                      hA, lda, dA, lda,
                      hL, ldl, dL, ldl,
                      ipiv,
...
CODELETS(ststrf, 4, cl_ststrf_cpu_func, cl_ststrf_cuda_func, 0)
#else
CODELETS_CPU(ststrf, 4, cl_ststrf_cpu_func)
#endif
