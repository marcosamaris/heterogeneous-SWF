...
{
    (void)nb;
    struct starpu_codelet *codelet = &cl_csymm;
    void (*callback)(void*) = options->profiling ? cl_csymm_callback : NULL;
...
static void cl_csymm_cpu_func(void *descr[], void *cl_arg)
{
    MORSE_enum side;
    MORSE_enum uplo;
    int M;
...
static void cl_csymm_cuda_func(void *descr[], void *cl_arg)
{
    MORSE_enum side;
    MORSE_enum uplo;
    int M;
    ...
    if (side == MorseLeft){
        ...
        fprintf(stderr, "Error in cl_csymm_cuda_func: bad side parameter %d\n", side);
    }
    if (uplo == MorseUpper){
        ...
        fprintf(stderr, "Error in cl_csymm_cuda_func: bad uplo parameter %d\n", uplo);
    }

    stat = cublasCsymm(handle,
 ...
 * Codelet definition
 */
CODELETS(csymm, 3, cl_csymm_cpu_func, cl_csymm_cuda_func, STARPU_CUDA_ASYNC)
