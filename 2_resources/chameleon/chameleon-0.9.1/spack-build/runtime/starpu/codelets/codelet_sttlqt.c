...
{
    (void)nb;
    struct starpu_codelet *codelet = &cl_sttlqt;
    void (*callback)(void*) = options->profiling ? cl_sttlqt_callback : NULL;
...
static void cl_sttlqt_cpu_func(void *descr[], void *cl_arg)
{
    int m;
    int n;
    int ib;
    ...
    TAU = (float *)STARPU_MATRIX_GET_PTR(descr[3]); /* nb * (ib+1) */
 ...
 * Codelet definition
 */
CODELETS_CPU(sttlqt, 4, cl_sttlqt_cpu_func)
