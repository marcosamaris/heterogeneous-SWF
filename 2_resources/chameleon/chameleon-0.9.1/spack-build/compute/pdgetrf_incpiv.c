 ...
 **/
void morse_pdgetrf_incpiv(MORSE_desc_t *A, MORSE_desc_t *L, int *IPIV,
                          MORSE_sequence_t *sequence, MORSE_request_t *request)
{
    ...
    h_work_size  = sizeof(double)*( 2*ib + 2*L->nb )*2*A->mb;
    d_work_size  = sizeof(double)*(   ib           )*2*A->mb;
#else
    h_work_size  = sizeof(double)*( ib*L->nb );
    d_work_size  = 0;
#endif
    ...
    morse_ddesc_alloc2(*DIAG, A->mb, A->nb, min(A->m, A->n), A->nb, 0, 0, min(A->m, A->n), A->nb);

    for (k = 0; k < minMNT; k++) {
        tempkm = k == A->mt-1 ? A->m-k*A->mb : A->mb;
        tempkn = k == A->nt-1 ? A->n-k*A->nb : A->nb;
        ldak = BLKLDD(A, k);
        MORSE_TASK_dgetrf_incpiv(
            &options,
