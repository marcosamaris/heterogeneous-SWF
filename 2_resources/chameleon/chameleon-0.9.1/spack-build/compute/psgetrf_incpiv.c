 ...
 **/
void morse_psgetrf_incpiv(MORSE_desc_t *A, MORSE_desc_t *L, int *IPIV,
                          MORSE_sequence_t *sequence, MORSE_request_t *request)
{
    ...
    h_work_size  = sizeof(float)*( 2*ib + 2*L->nb )*2*A->mb;
    d_work_size  = sizeof(float)*(   ib           )*2*A->mb;
#else
    h_work_size  = sizeof(float)*( ib*L->nb );
    d_work_size  = 0;
#endif
