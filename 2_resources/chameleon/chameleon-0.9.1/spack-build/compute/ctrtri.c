 ...
 *
 *  MORSE_ctrtri - Computes the inverse of a complex upper or lower
 *  triangular matrix A.
 *
 *******************************************************************************
 *
 ...
 *
 ******************************************************************************/
int MORSE_ctrtri(MORSE_enum uplo, MORSE_enum diag, int N,
                  MORSE_Complex32_t *A, int LDA)
{
    ...
    if (morse == NULL) {
        morse_fatal_error("MORSE_ctrtri", "MORSE not initialized");
        return MORSE_ERR_NOT_INITIALIZED;
    }
    /* Check input arguments */
    if (uplo != MorseUpper && uplo != MorseLower) {
        morse_error("MORSE_ctrtri", "illegal value of uplo");
        return -1;
    }
    if (diag != MorseUnit && diag != MorseNonUnit) {
        morse_error("MORSE_ctrtri", "illegal value of diag");
        return -2;
    }
    if (N < 0) {
        morse_error("MORSE_ctrtri", "illegal value of N");
        return -3;
    }
    if (LDA < max(1, N)) {
        morse_error("MORSE_ctrtri", "illegal value of LDA");
        return -5;
    }
    /* Quick return */
    if (max(N, 0) == 0)
    ...
    if (status != MORSE_SUCCESS) {
        morse_error("MORSE_ctrtri", "morse_tune() failed");
        return status;
    }
...
/*    }*/
    ...
    MORSE_ctrtri_Tile_Async(uplo, diag, &descA, sequence, &request);
 ...
 *
 *  MORSE_ctrtri_Tile - Computes the inverse of a complex upper or
 *  lower triangular matrix A.
 *  Tile equivalent of MORSE_ctrtri().
...
int MORSE_ctrtri_Tile(MORSE_enum uplo, MORSE_enum diag, MORSE_desc_t *A)
{
    ...
    morse = morse_context_self();
    if (morse == NULL) {
        morse_fatal_error("MORSE_ctrtri_Tile", "MORSE not initialized");
        return MORSE_ERR_NOT_INITIALIZED;
    }
    morse_sequence_create(morse, &sequence);
    MORSE_ctrtri_Tile_Async(uplo, diag, A, sequence, &request);
    RUNTIME_barrier(morse);
 ...
 *
 *  MORSE_ctrtri_Tile_Async - Computes the inverse of a complex upper or lower
 *  triangular matrix A.
 *  Non-blocking equivalent of MORSE_ctrtri_Tile().
 ...
 *
 ******************************************************************************/
int MORSE_ctrtri_Tile_Async(MORSE_enum uplo, MORSE_enum diag, MORSE_desc_t *A,
                             MORSE_sequence_t *sequence, MORSE_request_t *request)
{
    ...
    if (sequence == NULL) {
        morse_fatal_error("MORSE_ctrtri_Tile", "NULL sequence");
        return MORSE_ERR_UNALLOCATED;
    }
    if (request == NULL) {
        morse_fatal_error("MORSE_ctrtri_Tile", "NULL request");
        return MORSE_ERR_UNALLOCATED;
    }
    ...
    if (morse_desc_check(A) != MORSE_SUCCESS) {
        morse_error("MORSE_ctrtri_Tile", "invalid descriptor");
        return morse_request_fail(sequence, request, MORSE_ERR_ILLEGAL_VALUE);
    }
    /* Check input arguments */
    if (A->nb != A->mb) {
        morse_error("MORSE_ctrtri_Tile", "only square tiles supported");
        return morse_request_fail(sequence, request, MORSE_ERR_ILLEGAL_VALUE);
    }
    if (uplo != MorseUpper && uplo != MorseLower) {
        morse_error("MORSE_ctrtri_Tile", "illegal value of uplo");
        return morse_request_fail(sequence, request, -1);
    }
    if (diag != MorseUnit && diag != MorseNonUnit) {
        morse_error("MORSE_ctrtri_Tile", "illegal value of diag");
...
/*
    if (max(N, 0) == 0)
        return MORSE_SUCCESS;
*/
    morse_pctrtri(uplo, diag, A, sequence, request);
