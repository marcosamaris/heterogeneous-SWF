 ...
 * @ingroup float
 *
 *  MORSE_sormlq - Overwrites the general real M-by-N matrix C with
 * 
 *                  SIDE = 'L'     SIDE = 'R'
 *  TRANS = 'N':      Q * C          C * Q
 *  TRANS = 'C':      Q**T * C       C * Q**T
 ...
 *
 ******************************************************************************/
int MORSE_sormlq(MORSE_enum side, MORSE_enum trans, int M, int N, int K,
                  float *A, int LDA,
                  MORSE_desc_t *descT,
                  float *C, int LDC)
{
    ...
    if (morse == NULL) {
        morse_fatal_error("MORSE_sormlq", "MORSE not initialized");
    ...
    if ((side != MorseLeft) && (side != MorseRight)) {
        morse_error("MORSE_sormlq", "illegal value of side");
        return -1;
    }
    if ((trans != MorseTrans) && (trans != MorseNoTrans)){
        morse_error("MORSE_sormlq", "illegal value of trans");
        return -2;
    }
    if (M < 0) {
        morse_error("MORSE_sormlq", "illegal value of M");
        return -3;
    }
    if (N < 0) {
        morse_error("MORSE_sormlq", "illegal value of N");
        return -4;
    }
    if ((K < 0) || (K > An)) {
        morse_error("MORSE_sormlq", "illegal value of K");
        return -5;
    }
    if (LDA < max(1, K)) {
        morse_error("MORSE_sormlq", "illegal value of LDA");
        return -7;
    }
    if (LDC < max(1, M)) {
        morse_error("MORSE_sormlq", "illegal value of LDC");
        return -10;
    }
    ...
    if (status != MORSE_SUCCESS) {
        morse_error("MORSE_sormlq", "morse_tune() failed");
        return status;
    }
 ...
 * @ingroup float_Tile
 *
 *  MORSE_sormlq_Tile - overwrites the general M-by-N matrix C with Q*C, where Q is an orthogonal
 *  matrix (unitary in the real case) defined as the product of elementary reflectors returned
 *  by MORSE_sgelqf_Tile Q is of order M.
 ...
 *
 ******************************************************************************/
int MORSE_sormlq_Tile(MORSE_enum side, MORSE_enum trans,
                       MORSE_desc_t *A, MORSE_desc_t *T, MORSE_desc_t *C)
{
    ...
    morse = morse_context_self();
    if (morse == NULL) {
        morse_fatal_error("MORSE_sormlq_Tile", "MORSE not initialized");
        return MORSE_ERR_NOT_INITIALIZED;
    }
    morse_sequence_create(morse, &sequence);
    MORSE_sormlq_Tile_Async(side, trans, A, T, C, sequence, &request);
    RUNTIME_barrier(morse);
 ...
 *
 *  Non-blocking equivalent of MORSE_sormlq_Tile().
 ...
 *
 ******************************************************************************/
int MORSE_sormlq_Tile_Async(MORSE_enum side, MORSE_enum trans,
                             MORSE_desc_t *A, MORSE_desc_t *T, MORSE_desc_t *C,
                             MORSE_sequence_t *sequence, MORSE_request_t *request)
{
    ...
    if (sequence == NULL) {
        morse_fatal_error("MORSE_sormlq_Tile", "NULL sequence");
        return MORSE_ERR_UNALLOCATED;
    }
    if (request == NULL) {
        morse_fatal_error("MORSE_sormlq_Tile", "NULL request");
        return MORSE_ERR_UNALLOCATED;
    }
    ...
    if (morse_desc_check(A) != MORSE_SUCCESS) {
        morse_error("MORSE_sormlq_Tile", "invalid first descriptor");
        return morse_request_fail(sequence, request, MORSE_ERR_ILLEGAL_VALUE);
    }
    if (morse_desc_check(T) != MORSE_SUCCESS) {
        morse_error("MORSE_sormlq_Tile", "invalid second descriptor");
        return morse_request_fail(sequence, request, MORSE_ERR_ILLEGAL_VALUE);
    }
    if (morse_desc_check(C) != MORSE_SUCCESS) {
        morse_error("MORSE_sormlq_Tile", "invalid third descriptor");
        return morse_request_fail(sequence, request, MORSE_ERR_ILLEGAL_VALUE);
    }
    /* Check input arguments */
    if (A->nb != A->mb || C->nb != C->mb) {
        morse_error("MORSE_sormlq_Tile", "only square tiles supported");
        return morse_request_fail(sequence, request, MORSE_ERR_ILLEGAL_VALUE);
    }
    if ((side != MorseLeft) && (side != MorseRight)) {
        return morse_request_fail(sequence, request, MORSE_ERR_ILLEGAL_VALUE);
...
*/
    if (morse->householder == MORSE_FLAT_HOUSEHOLDER) {
        if ( (trans == MorseTrans) &&
             (side == MorseLeft) ) {
            morse_psormlq(side, trans, A, C, T, sequence, request);
        } else {
            morse_psormlq(side, trans, A, C, T, sequence, request);
        }
    }
    else {
        morse_psormlqrh(side, trans, A, C, T, MORSE_RHBLK, sequence, request);
    }
