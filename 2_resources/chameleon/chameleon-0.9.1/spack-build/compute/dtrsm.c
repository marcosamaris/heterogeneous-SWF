 ...
 * @ingroup double
 *
 *  MORSE_dtrsm - Computes triangular solve A*X = B or X*A = B.
 *
 *******************************************************************************
 *
 * @param[in] side
 ...
 *
 ******************************************************************************/
int MORSE_dtrsm(MORSE_enum side, MORSE_enum uplo,
                 ...
                 double *B, int LDB)
{
    ...
    if (morse == NULL) {
        morse_fatal_error("MORSE_dtrsm", "MORSE not initialized");
    ...
    if (side != MorseLeft && side != MorseRight) {
        morse_error("MORSE_dtrsm", "illegal value of side");
        return -1;
    }
    if (uplo != MorseUpper && uplo != MorseLower) {
        morse_error("MORSE_dtrsm", "illegal value of uplo");
        return -2;
    }
    if (transA != MorseTrans && transA != MorseNoTrans && transA != MorseTrans ) {
        morse_error("MORSE_dtrsm", "illegal value of transA");
        return -3;
    }
    if (diag != MorseUnit && diag != MorseNonUnit) {
        morse_error("MORSE_dtrsm", "illegal value of diag");
        return -4;
    }
    if (N < 0) {
        morse_error("MORSE_dtrsm", "illegal value of N");
        return -5;
    }
    if (NRHS < 0) {
        morse_error("MORSE_dtrsm", "illegal value of NRHS");
        return -6;
    }
    if (LDA < max(1, NA)) {
        morse_error("MORSE_dtrsm", "illegal value of LDA");
        return -8;
    }
    if (LDB < max(1, N)) {
        morse_error("MORSE_dtrsm", "illegal value of LDB");
        return -10;
    }
    /* Quick return */
    if (min(N, NRHS) == 0)
    ...
    if (status != MORSE_SUCCESS) {
        morse_error("MORSE_dtrsm", "morse_tune() failed");
        return status;
    }
 ...
 * @ingroup double_Tile
 *
 *  MORSE_dtrsm_Tile - Computes triangular solve.
 *  Tile equivalent of MORSE_dtrsm().
 ...
 *
 ******************************************************************************/
int MORSE_dtrsm_Tile(MORSE_enum side, MORSE_enum uplo,
                      MORSE_enum transA, MORSE_enum diag,
                      double alpha, MORSE_desc_t *A, MORSE_desc_t *B)
{
    ...
    morse = morse_context_self();
    if (morse == NULL) {
        morse_fatal_error("MORSE_dtrsm_Tile", "MORSE not initialized");
        return MORSE_ERR_NOT_INITIALIZED;
    }
    morse_sequence_create(morse, &sequence);
 ...
 *
 *  MORSE_dtrsm_Tile_Async - Computes triangular solve.
 *  Non-blocking equivalent of MORSE_dtrsm_Tile().
 ...
 *
 ******************************************************************************/
int MORSE_dtrsm_Tile_Async(MORSE_enum side, MORSE_enum uplo,
                            MORSE_enum transA, MORSE_enum diag,
                            double alpha, MORSE_desc_t *A, MORSE_desc_t *B,
                            MORSE_sequence_t *sequence, MORSE_request_t *request)
{
    ...
    if (sequence == NULL) {
        morse_fatal_error("MORSE_dtrsm_Tile", "NULL sequence");
        return MORSE_ERR_UNALLOCATED;
    }
    if (request == NULL) {
        morse_fatal_error("MORSE_dtrsm_Tile", "NULL request");
        return MORSE_ERR_UNALLOCATED;
    }
    ...
    if (morse_desc_check(A) != MORSE_SUCCESS) {
        morse_error("MORSE_dtrsm_Tile", "invalid first descriptor");
        return morse_request_fail(sequence, request, MORSE_ERR_ILLEGAL_VALUE);
    }
    if (morse_desc_check(B) != MORSE_SUCCESS) {
        morse_error("MORSE_dtrsm_Tile", "invalid second descriptor");
        return morse_request_fail(sequence, request, MORSE_ERR_ILLEGAL_VALUE);
    }
    /* Check input arguments */
    if (A->nb != A->mb || B->nb != B->mb) {
        morse_error("MORSE_dtrsm_Tile", "only square tiles supported");
        return morse_request_fail(sequence, request, MORSE_ERR_ILLEGAL_VALUE);
    }
    if (side != MorseLeft && side != MorseRight) {
        morse_error("MORSE_dtrsm_Tile", "illegal value of side");
        return morse_request_fail(sequence, request, -1);
    }
    if (uplo != MorseUpper && uplo != MorseLower) {
        morse_error("MORSE_dtrsm_Tile", "illegal value of uplo");
        return morse_request_fail(sequence, request, -2);
    }
    if (transA != MorseTrans && transA != MorseNoTrans && transA != MorseTrans) {
        morse_error("MORSE_dtrsm_Tile", "illegal value of transA");
        return morse_request_fail(sequence, request, -3);
    }
    if (diag != MorseUnit && diag != MorseNonUnit) {
        morse_error("MORSE_dtrsm_Tile", "illegal value of diag");
        return morse_request_fail(sequence, request, -4);
    }

    /* Quick return */
    morse_pdtrsm(side, uplo, transA, diag, alpha, A, B, sequence, request);
