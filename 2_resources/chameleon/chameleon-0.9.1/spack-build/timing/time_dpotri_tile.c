...
#define _NAME  "MORSE_dpotri_Tile"
...
RunTest(int *iparam, double *dparam, morse_time_t *t_) 
{
...
#if defined(TRACE_BY_SEQUENCE)
    {
        ...
        if ( ! iparam[IPARAM_ASYNC] ) {
            ...
            MORSE_dpotrf_Tile_Async(uplo, descA,                sequence[0], &request[0]);
            MORSE_Sequence_Wait(sequence[0]);

            MORSE_dtrtri_Tile_Async(uplo, MorseNonUnit, descA, sequence[1], &request[1]);
            MORSE_Sequence_Wait(sequence[1]);

            MORSE_dlauum_Tile_Async(uplo, descA,                sequence[2], &request[2]);
...
#else
    {
        if ( ! iparam[IPARAM_ASYNC] ) {
            ...
            MORSE_dtrtri_Tile(uplo, MorseNonUnit, descA);
            MORSE_dlauum_Tile(uplo, descA);
            STOP_TIMING();
            ...
            MORSE_dpotrf_Tile_Async(uplo, descA, sequence, &request[0]);
            MORSE_dpotri_Tile_Async(uplo, descA, sequence, &request[1]);
