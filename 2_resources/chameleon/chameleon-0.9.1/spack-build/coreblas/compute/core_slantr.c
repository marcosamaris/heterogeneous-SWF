 ...
 *
 *  CORE_PLASMA_slantr returns the value
 *
 ...
 *          If uplo == MorseUpper, M <= N. When M = 0, CORE_slantr returns 0.
 *
 * @param[in] N
 *          The number of columns of the matrix A. N >= 0.
 *          If uplo == MorseLower, N <= M. When N = 0, CORE_slantr returns 0.
 *
 * @param[in] A
...
void CORE_slantr(MORSE_enum norm, MORSE_enum uplo, MORSE_enum diag,
                 int M, int N,
                 ...
                 float *work, float *normA)
{
    ...
    *normA = LAPACKE_slantr_work(
    ...
    case MorseOneNorm:
        CORE_strasm( MorseColumnwise, uplo, diag, M, N,
                     A, LDA, work );
        if ( uplo == MorseLower )
            N = min(M,N);
    ...
    case MorseInfNorm:
        CORE_strasm( MorseRowwise, uplo, diag, M, N,
                     A, LDA, work );
        if ( uplo == MorseUpper )
            M = min(M,N);
