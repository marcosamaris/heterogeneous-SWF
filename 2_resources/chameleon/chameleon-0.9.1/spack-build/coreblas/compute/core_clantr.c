 ...
 *
 *  CORE_PLASMA_clantr returns the value
 *
 ...
 *          If uplo == MorseUpper, M <= N. When M = 0, CORE_clantr returns 0.
 *
 * @param[in] N
 *          The number of columns of the matrix A. N >= 0.
 *          If uplo == MorseLower, N <= M. When N = 0, CORE_clantr returns 0.
 *
 * @param[in] A
...
void CORE_clantr(MORSE_enum norm, MORSE_enum uplo, MORSE_enum diag,
                 int M, int N,
                 ...
                 float *work, float *normA)
{
    ...
    *normA = LAPACKE_clantr_work(
    ...
    case MorseOneNorm:
        CORE_ctrasm( MorseColumnwise, uplo, diag, M, N,
                     A, LDA, work );
        if ( uplo == MorseLower )
            N = min(M,N);
    ...
    case MorseInfNorm:
        CORE_ctrasm( MorseRowwise, uplo, diag, M, N,
                     A, LDA, work );
        if ( uplo == MorseUpper )
            M = min(M,N);
