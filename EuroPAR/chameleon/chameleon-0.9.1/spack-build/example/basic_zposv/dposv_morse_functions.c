 ...
 * @date 2014-10-13
 * @generated d Tue Sep 26 20:43:19 2017
 *
 **/
...
int main(int argc, char *argv[]) {
    ...
    MORSE_dposv_Tile(iparam[IPARAM_UPLO], descA, descX);
    /* note that this call is equivalent to
     * MORSE_dpotrf_Tile(uplo, descA) followed by
     * MORSE_dpotrs_Tile(uplo, descA, descX )
     * */

    /* check if solve is correct i.e. AX-B = 0 */
    /* compute norms to check the result */
    double anorm = MORSE_dlange_Tile(MorseInfNorm, descAC);
    double bnorm = MORSE_dlange_Tile(MorseInfNorm, descB);
    double xnorm = MORSE_dlange_Tile(MorseInfNorm, descX);

    /* compute A*X-B, store the result in B */
    MORSE_dgemm_Tile( MorseNoTrans, MorseNoTrans, 1.0, descAC, descX, -1.0, descB );
    double res = MORSE_dlange_Tile(MorseInfNorm, descB);
