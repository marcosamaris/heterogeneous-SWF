 ...
 **/
MORSE_Complex32_t **allocate_tile_matrix(int m, int n, int nb){
    int i, j;
    int mt, nt;
    MORSE_Complex32_t **mat;

    /* compute number of tiles in rows and columns */
    mt = (m%nb==0) ? (m/nb) : (m/nb+1);
    nt = (n%nb==0) ? (n/nb) : (n/nb+1);
    mat = malloc( mt*nt*sizeof(MORSE_Complex32_t*) );
    if (!mat){
        printf ("\nIn allocate_tile_matrix, memory Allocation Failure of mat !\n\n");
        exit (EXIT_FAILURE);
    }
    for (i=0; i<mt*nt; i++){
        *(mat+i) = calloc( nb*nb, sizeof(MORSE_Complex32_t) );
        if (!*(mat+i)){
...
int main(int argc, char *argv[]) {
     ...
     * pointers to tiles
     */
    MORSE_Desc_Create_User(&descA, matA, MorseComplexFloat,
