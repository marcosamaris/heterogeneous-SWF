 ...
 * @date 2014-10-13
 * @generated s Tue Sep 26 20:43:19 2017
 *
 **/
 ...
 **/
float **allocate_tile_matrix(int m, int n, int nb){
    int i, j;
    int mt, nt;
    float **mat;
    ...
    mat = malloc( mt*nt*sizeof(float*) );
    if (!mat){
        printf ("\nIn allocate_tile_matrix, memory Allocation Failure of mat !\n\n");
        exit (EXIT_FAILURE);
    }
    for (i=0; i<mt*nt; i++){
        *(mat+i) = calloc( nb*nb, sizeof(float) );
        if (!*(mat+i)){
...
int main(int argc, char *argv[]) {
     ...
     * pointers to tiles
     */
    MORSE_Desc_Create_User(&descA, matA, MorseRealFloat,
    ...
    MORSE_sposv_Tile(iparam[IPARAM_UPLO], descA, descX);
    /* note that this call is equivalent to
     * MORSE_spotrf_Tile(uplo, descA) followed by
     * MORSE_spotrs_Tile(uplo, descA, descX )
     * */

    /* check if solve is correct i.e. AX-B = 0 */
    /* compute norms to check the result */
    float anorm = MORSE_slange_Tile(MorseInfNorm, descAC);
    float bnorm = MORSE_slange_Tile(MorseInfNorm, descB);
    float xnorm = MORSE_slange_Tile(MorseInfNorm, descX);

    /* compute A*X-B, store the result in B */
    MORSE_sgemm_Tile( MorseNoTrans, MorseNoTrans, 1.0, descAC, descX, -1.0, descB );
    float res = MORSE_slange_Tile(MorseInfNorm, descB);
