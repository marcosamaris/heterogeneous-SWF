...
int testing_ssyr2k(int argc, char **argv)
{
    ...
    printf("\n");
    printf("------ TESTS FOR CHAMELEON SSYR2K ROUTINE -------  \n");
    ...
    */
    ...
    for (u=0; u<2; u++) {
        for (t=0; t<2; t++) {
            ...
            /* MORSE SSYR2K */
            MORSE_ssyr2k(uplo[u], trans[t], N, K, alpha, A, LDA, B, LDB, beta, Cfinal, LDC);
                          ...
                          float beta, float *Cref, float *Cmorse, int LDC)
{
    ...
    Bnorm       = LAPACKE_slange_work(LAPACK_COL_MAJOR, morse_lapack_const(MorseInfNorm), 
                                      (trans == MorseNoTrans) ? N : K, 
