...
static int check_solution(MORSE_enum side, MORSE_enum uplo, MORSE_enum trans, MORSE_enum diag,
                          ...
                          MORSE_Complex32_t *Bref, MORSE_Complex32_t *Bmorse, int LDB);
...
int testing_ctrmm(int argc, char **argv)
{
    ...
    MORSE_Complex32_t *Binit  = (MORSE_Complex32_t *)malloc(LDBxN*sizeof(MORSE_Complex32_t));
    MORSE_Complex32_t *Bfinal = (MORSE_Complex32_t *)malloc(LDBxN*sizeof(MORSE_Complex32_t));
    ...
    printf("\n");
    printf("------ TESTS FOR CHAMELEON CTRMM ROUTINE -------  \n");
        ...
        for (u=0; u<2; u++) {
            ...
            for (t=0; t<2; t++) {
                ...
                for (d=0; d<2; d++) {
                    ...
                    memcpy(Binit,  B, LDBxN*sizeof(MORSE_Complex32_t));
                    memcpy(Bfinal, B, LDBxN*sizeof(MORSE_Complex32_t));

                    /* MORSE CTRMM */
                    MORSE_ctrmm(side[s], uplo[u], trans[t], diag[d],
                          ...
                          MORSE_Complex32_t *Bref, MORSE_Complex32_t *Bmorse, int LDB)
{
    ...
    float eps;
    MORSE_Complex32_t mzone = (MORSE_Complex32_t)-1.0;
    ...
    Anorm       = LAPACKE_clantr_work(LAPACK_COL_MAJOR, morse_lapack_const(MorseInfNorm), morse_lapack_const(uplo), morse_lapack_const(diag),
                                Am, An, A, LDA, work);
    Binitnorm   = LAPACKE_clange_work(LAPACK_COL_MAJOR, morse_lapack_const(MorseInfNorm), M, N, Bref,    LDB, work);
    Bmorsenorm = LAPACKE_clange_work(LAPACK_COL_MAJOR, morse_lapack_const(MorseInfNorm), M, N, Bmorse, LDB, work);
    ...
    Blapacknorm = LAPACKE_clange_work(LAPACK_COL_MAJOR, morse_lapack_const(MorseInfNorm), M, N, Bref, LDB, work);

    cblas_caxpy(LDB * N, CBLAS_SADDR(mzone), Bmorse, 1, Bref, 1);

    Rnorm = LAPACKE_clange_work(LAPACK_COL_MAJOR, morse_lapack_const(MorseInfNorm), M, N, Bref, LDB, work);
