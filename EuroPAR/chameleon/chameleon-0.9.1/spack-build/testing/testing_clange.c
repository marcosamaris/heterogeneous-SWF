...
int testing_clange(int argc, char **argv)
{
    ...
    /* Allocate Data */
    MORSE_Complex32_t *A    = (MORSE_Complex32_t *)malloc(LDAxN*sizeof(MORSE_Complex32_t));
    ...
    printf("\n");
    printf("------ TESTS FOR CHAMELEON CLANGE ROUTINE -------  \n");
    ...
    /* Initialize A, B, C */
    MORSE_cplrnt( M, N, A, LDA, 3436 );

    /* MORSE CLANGE */
    for(n=0; n<4; n++) {
        normmorse  = MORSE_clange(norm[n], M, N, A, LDA);
        normlapack = LAPACKE_clange_work(LAPACK_COL_MAJOR, morse_lapack_const(norm[n]), M, N, A, LDA, work);
        ...
        for(n=1; n<3; n++) {
            for(u=0; u<2; u++) {
                int d;
                for(d=0; d<2; d++) {
                    normmorse = MORSE_clantr(norm[n], uplo[u], diag[d], M, N, A, LDA);
                    normlapack = LAPACKE_clantr_work(LAPACK_COL_MAJOR, morse_lapack_const(norm[n]), morse_lapack_const(uplo[u]),
    ...
    for(n=0; n<4; n++) {
        for(u=0; u<2; u++) {
            normmorse = MORSE_clansy(norm[n], uplo[u], min(M,N), A, LDA);
            normlapack = LAPACKE_clansy_work(LAPACK_COL_MAJOR, morse_lapack_const(norm[n]), morse_lapack_const(uplo[u]), min(M,N), A, LDA, work);
            printf("LAPACK %e, CHAMELEON %e\n", normlapack, normmorse);
        ...
        for(n=0; n<4; n++) {
            for(u=0; u<2; u++) {
                normmorse = MORSE_clanhe(norm[n], uplo[u], min(M,N), A, LDA);
                normlapack = LAPACKE_clanhe_work(LAPACK_COL_MAJOR, morse_lapack_const(norm[n]), morse_lapack_const(uplo[u]), min(M,N), A, LDA, work);
                printf("LAPACK %e, CHAMELEON %e\n", normlapack, normmorse);
