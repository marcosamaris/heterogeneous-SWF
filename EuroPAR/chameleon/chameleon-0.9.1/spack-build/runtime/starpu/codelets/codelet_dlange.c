...
{
    (void)NB;
    struct starpu_codelet *codelet = &cl_dlange;
...
static void cl_dlange_cpu_func(void *descr[], void *cl_arg)
{
    double *normA;
    MORSE_enum norm;
    int M;
    ...
    CORE_dlange( norm, M, N, A, LDA, work, normA);
}

/*
 * Codelet definition
 */
CODELETS_CPU(dlange, 3, cl_dlange_cpu_func)

void MORSE_TASK_dlange_max(MORSE_option_t *options,
                           MORSE_desc_t *A, int Am, int An,
                           MORSE_desc_t *B, int Bm, int Bn)
{
    struct starpu_codelet *codelet = &cl_dlange_max;
    void (*callback)(void*) = options->profiling ? cl_dlange_callback : NULL;
    if ( morse_desc_islocal( A, Am, An ) ||
         morse_desc_islocal( B, Bm, Bn ) ){
        starpu_insert_task(codelet,
...
static void cl_dlange_max_cpu_func(void *descr[], void *cl_arg)
{
    double *A;
    double *normA;
 ...
 * Codelet definition
 */
CODELETS_CPU(dlange_max, 2, cl_dlange_max_cpu_func)
