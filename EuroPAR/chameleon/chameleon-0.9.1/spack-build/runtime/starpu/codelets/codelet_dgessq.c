...
{
    struct starpu_codelet *codelet = &cl_dgessq;
...
static void cl_dgessq_cpu_func(void *descr[], void *cl_arg)
{
    int m;
    int n;
    double *A;
    ...
    CORE_dgessq( m, n, A, lda, &SCALESUMSQ[0], &SCALESUMSQ[1] );
}
/*
 * Codelet definition
 */
CODELETS_CPU(dgessq, 2, cl_dgessq_cpu_func)
