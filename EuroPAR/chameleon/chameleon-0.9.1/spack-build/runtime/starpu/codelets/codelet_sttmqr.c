 ...
 *
 *  CORE_sttmqr overwrites the general real M1-by-N1 tile A1 and
...
{
    (void)nb;
    struct starpu_codelet *codelet = &cl_sttmqr;
    void (*callback)(void*) = options->profiling ? cl_sttmqr_callback : NULL;
...
static void cl_sttmqr_cpu_func(void *descr[], void *cl_arg)
{
    MORSE_enum side;
    MORSE_enum trans;
    int m1;
    ...
    WORK = (float *)STARPU_MATRIX_GET_PTR(descr[4]); /* nb * ib */
 ...
 * Codelet definition
 */
CODELETS_CPU(sttmqr, 5, cl_sttmqr_cpu_func)
