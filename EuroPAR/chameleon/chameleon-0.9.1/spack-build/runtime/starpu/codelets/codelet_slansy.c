...
{
    (void)NB;
    struct starpu_codelet *codelet = &cl_slansy;
...
static void cl_slansy_cpu_func(void *descr[], void *cl_arg)
{
    float *normA;
    MORSE_enum norm;
    MORSE_enum uplo;
    ...
    CORE_slansy( norm, uplo, N, A, LDA, work, normA);
}

/*
 * Codelet definition
 */
CODELETS_CPU(slansy, 3, cl_slansy_cpu_func)
