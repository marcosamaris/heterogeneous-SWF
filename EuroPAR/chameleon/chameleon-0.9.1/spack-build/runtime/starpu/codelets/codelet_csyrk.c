...
{
    (void)nb;
    struct starpu_codelet *codelet = &cl_csyrk;
    void (*callback)(void*) = options->profiling ? cl_csyrk_callback : NULL;
...
static void cl_csyrk_cpu_func(void *descr[], void *cl_arg)
{
    MORSE_enum uplo;
    MORSE_enum trans;
    int n;
...
static void cl_csyrk_cuda_func(void *descr[], void *cl_arg)
{
    MORSE_enum uplo;
    MORSE_enum trans;
    int n;
    ...
    if (uplo == MorseUpper){
        ...
        fprintf(stderr, "Error in cl_csyrk_cuda_func: bad uplo parameter %d\n", uplo);
    }
    if (trans == MorseNoTrans){
        ...
        fprintf(stderr, "Error in cl_csyrk_cuda_func: bad trans parameter %d\n", trans);
    }

    stat = cublasCsyrk(handle,
        cublasUplo, cublasTrans,
        n, k,
        (const cuFloatComplex *) &alpha, A, lda,
        (const cuFloatComplex *) &beta, C, ldc);
    if (stat != CUBLAS_STATUS_SUCCESS){
        printf ("cublasCsyrk failed");
 ...
 * Codelet definition
 */
CODELETS(csyrk, 2, cl_csyrk_cpu_func, cl_csyrk_cuda_func, STARPU_CUDA_ASYNC)
