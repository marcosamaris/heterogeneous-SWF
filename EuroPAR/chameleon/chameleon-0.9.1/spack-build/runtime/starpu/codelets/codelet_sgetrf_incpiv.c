...
{
    (void)nb;
    struct starpu_codelet *codelet = &cl_sgetrf_incpiv;
    void (*callback)(void*) = options->profiling ? cl_sgetrf_incpiv_callback : NULL;
...
static void cl_sgetrf_incpiv_cpu_func(void *descr[], void *cl_arg)
{
    ...
    L = (float *)STARPU_MATRIX_GET_PTR(descr[1]);
	...
	L += ib;
        for (i=0; i<n; i+=ib) {
            sb = min( ib, n-i );
            CORE_slacpy(MorseUpperLower, sb, sb, A+(i*lda+i), lda, L+(i*ldl), ldl );

            CORE_strtri( MorseLower, MorseUnit, sb, L+(i*ldl), ldl, &info );
            if (info != 0 ) {
...
static void cl_sgetrf_incpiv_cuda_func(void *descr[], void *cl_arg)
{
    int m;
    int n;
    int ib;
    ...
    dA = (float *)STARPU_MATRIX_GET_PTR(descr[0]);
    dL = (float *)STARPU_MATRIX_GET_PTR(descr[1]);
    /*
     * hwork => at least (IB+NB)*IB contains all hA and hL
     * dwork => at least IB*NB
     */
    hA    = (float*)RUNTIME_starpu_ws_getlocal(h_work);
    dwork = (float *)STARPU_MATRIX_GET_PTR(descr[2]);

    hL = hA + lda*ib;

    /* Initialize L to 0 */
    memset(hL, 0, ib*ib*sizeof(float));
    ...
    magma_sgetrf_incpiv_gpu( MagmaColMajor, m, n, ib,
                             hA, lda, dA, lda,
                             hL, ib,  dL, ldl,
                             IPIV,
                             dwork, lda,
...
CODELETS(sgetrf_incpiv, 3, cl_sgetrf_incpiv_cpu_func, cl_sgetrf_incpiv_cuda_func, 0)
#else
CODELETS_CPU(sgetrf_incpiv, 3, cl_sgetrf_incpiv_cpu_func)
#endif
