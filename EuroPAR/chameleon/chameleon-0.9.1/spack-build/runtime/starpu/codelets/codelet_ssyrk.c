...
{
    (void)nb;
    struct starpu_codelet *codelet = &cl_ssyrk;
    void (*callback)(void*) = options->profiling ? cl_ssyrk_callback : NULL;
...
static void cl_ssyrk_cpu_func(void *descr[], void *cl_arg)
{
    MORSE_enum uplo;
    MORSE_enum trans;
    int n;
    ...
    C = (float *)STARPU_MATRIX_GET_PTR(descr[1]);
...
static void cl_ssyrk_cuda_func(void *descr[], void *cl_arg)
{
    MORSE_enum uplo;
    MORSE_enum trans;
    int n;
    ...
    if (uplo == MorseUpper){
        ...
        fprintf(stderr, "Error in cl_ssyrk_cuda_func: bad uplo parameter %d\n", uplo);
    }
    if (trans == MorseNoTrans){
        ...
        fprintf(stderr, "Error in cl_ssyrk_cuda_func: bad trans parameter %d\n", trans);
    }

    stat = cublasSsyrk(handle,
 ...
 * Codelet definition
 */
CODELETS(ssyrk, 2, cl_ssyrk_cpu_func, cl_ssyrk_cuda_func, STARPU_CUDA_ASYNC)
