...
{
    struct starpu_codelet *codelet = &cl_strssq;
    void (*callback)(void*) = options->profiling ? cl_strasm_callback : NULL;
...
static void cl_strssq_cpu_func(void *descr[], void *cl_arg)
{
    MORSE_enum uplo;
    MORSE_enum diag;
    int m;
    ...
    CORE_strssq( uplo, diag, m, n, A, lda, &SCALESUMSQ[0], &SCALESUMSQ[1]);
}
/*
 * Codelet definition
 */
CODELETS_CPU(strssq, 2, cl_strssq_cpu_func)
