...
{
    (void)nb;
    struct starpu_codelet *codelet = &cl_chemm;
    void (*callback)(void*) = options->profiling ? cl_chemm_callback : NULL;
...
static void cl_chemm_cpu_func(void *descr[], void *cl_arg)
{
    MORSE_enum side;
    MORSE_enum uplo;
    int M;
...
static void cl_chemm_cuda_func(void *descr[], void *cl_arg)
{
    MORSE_enum side;
    MORSE_enum uplo;
    int M;
    ...
    if (side == MorseLeft){
        ...
        fprintf(stderr, "Error in cl_chemm_cuda_func: bad side parameter %d\n", side);
    }
    if (uplo == MorseUpper){
        ...
        fprintf(stderr, "Error in cl_chemm_cuda_func: bad uplo parameter %d\n", uplo);
    }

    stat = cublasChemm(handle,
 ...
 * Codelet definition
 */
CODELETS(chemm, 3, cl_chemm_cpu_func, cl_chemm_cuda_func, STARPU_CUDA_ASYNC)
