 ...
 *
 *  CORE_sgeqrt computes a QR factorization of a real M-by-N tile A:
 *  A = Q * R.
 *
...
{
    (void)nb;
    struct starpu_codelet *codelet = &cl_sgeqrt;
    void (*callback)(void*) = options->profiling ? cl_sgeqrt_callback : NULL;
         ...
         morse_desc_islocal( T, Tm, Tn ) )
    {
        starpu_insert_task(
            ...
            STARPU_W,         RTBLKADDR(T, float, Tm, Tn),
...
static void cl_sgeqrt_cpu_func(void *descr[], void *cl_arg)
{
    ...
    TAU = (float *)STARPU_MATRIX_GET_PTR(descr[2]); /* max(m,n) + n * ib */
...
magma_int_t
magma_sgemerge_gpu(magma_side_t side, magma_diag_t diag,
                   ...
                   CUstream stream)
{
    ...
    if (side == MagmaLeft){
        for(i=0; i<N; i++){
            cola = A + i*LDA;
            colb = B + i*LDB;
//            cublasScopy(handle, i+1, cola, 1, colb, 1);
        ...
        for(i=0; i<N; i++){
            cola = A + i*LDA;
            colb = B + i*LDB;
//            cublasScopy(handle, M-i, cola + i, 1, colb + i, 1);
...
//            cublasScopy(i+1, cola, 1, colb, 1);
        ...
        for(i=0; i<N; i++){
            cola = A + i*LDA;
            colb = B + i*LDB;
//            cublasScopy(M-i, cola + i, 1, colb + i, 1);
            cudaMemcpyAsync(colb+i , cola+i,
                            (M-i)*sizeof(float),
                            cudaMemcpyDeviceToDevice, stream);
        }
...
magma_int_t
magma_sgeqrt_gpu( magma_int_t m, magma_int_t n, magma_int_t nb,
                  float *da, magma_int_t ldda,
                  float *v,  magma_int_t ldv,
                  float *dt, magma_int_t lddt,
                  float *t,  magma_int_t ldt,
                  float *dd,
                  float *d,  magma_int_t ldd,
                  float *tau,
                  float *hwork,
                  float *dwork)
{
...
//    hwork[0] = *((float*) &lwkopt);
    ...
    memset(t_ref(0,0), 0, nb*nb*sizeof(float));
    cudaMemsetAsync(dt_ref(0,0), 0, nb*n*sizeof(float), stream);

    /* copy first panel of A on the host */
//	cublasGetMatrix(m, min(nb,n), sizeof(float),
//                    da_ref(0, 0), ldda,
//                    v, ldv);
    cudaMemcpy( v, da_ref(0,0),
                m*min(nb,n)*sizeof(float),
...
//                            v, ldv);
            ...
            magma_sgemerge_gpu(MagmaLeft, MagmaUnit, old_ib, old_ib,
        ...
        if ( i + ib < n ){
            ...
            CORE_sgesplit(MorseLeft, MorseUnit, min(rows,ib), ib,
                          (float*) v_ref(i,0), ldv,
                          (float*) d, ldd);

            /* copy from host to device a tile diag */
            cublasSetMatrix( min(rows,ib), ib, sizeof(float),
                             d, ldd, dd, ldd );
        }
        ...
        /* A panel (with zeros in upper tri of its diag) is ready to be used
         in input of slarfb_gpu: we send the panel to the gpu */
...
static void cl_sgeqrt_cuda_func(void *descr[], void *cl_arg)
{
    MORSE_starpu_ws_t *h_work;
    int m;
    int n;
    int ib;
    float *h_A, *h_T, *h_D, *h_W, *h_TAU;
    float *d_A, *d_T, *d_D, *d_W;
    int lda, ldt;

    starpu_codelet_unpack_args(cl_arg, &m, &n, &ib, &lda, &ldt, &h_work);

    /* Gather pointer to data on device */
    d_A = (float *)STARPU_MATRIX_GET_PTR(descr[0]);
    d_T = (float *)STARPU_MATRIX_GET_PTR(descr[1]);
    d_W = (float *)STARPU_MATRIX_GET_PTR(descr[2]); /* ib*n + ib * ib*/
    d_D = d_W + ib*n;

    /* scratch data on host */
    /* m*ib + ib*ib + max(m,n) + ib*ib + ib*ib */
    h_A = (float*)RUNTIME_starpu_ws_getlocal(h_work);
...
CODELETS(sgeqrt, 3, cl_sgeqrt_cpu_func, cl_sgeqrt_cuda_func, 0)
#else
CODELETS_CPU(sgeqrt, 3, cl_sgeqrt_cpu_func)
#endif
