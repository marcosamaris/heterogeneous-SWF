...
{
    (void)NB;
    struct starpu_codelet *codelet = &cl_clange;
...
static void cl_clange_cpu_func(void *descr[], void *cl_arg)
{
    float *normA;
    MORSE_enum norm;
    int M;
    ...
    CORE_clange( norm, M, N, A, LDA, work, normA);
}

/*
 * Codelet definition
 */
CODELETS_CPU(clange, 3, cl_clange_cpu_func)

void MORSE_TASK_clange_max(MORSE_option_t *options,
                           MORSE_desc_t *A, int Am, int An,
                           MORSE_desc_t *B, int Bm, int Bn)
{
    struct starpu_codelet *codelet = &cl_clange_max;
    void (*callback)(void*) = options->profiling ? cl_clange_callback : NULL;
    if ( morse_desc_islocal( A, Am, An ) ||
         morse_desc_islocal( B, Bm, Bn ) ){
        starpu_insert_task(codelet,
...
static void cl_clange_max_cpu_func(void *descr[], void *cl_arg)
{
    float *A;
    float *normA;
 ...
 * Codelet definition
 */
CODELETS_CPU(clange_max, 2, cl_clange_max_cpu_func)
