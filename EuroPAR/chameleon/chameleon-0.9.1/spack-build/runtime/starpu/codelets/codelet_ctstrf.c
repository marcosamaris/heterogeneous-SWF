...
{
    (void)nb;
    struct starpu_codelet *codelet = &cl_ctstrf;
    void (*callback)(void*) = options->profiling ? cl_ctstrf_callback : NULL;
         ...
         morse_desc_islocal( L, Lm, Ln ) )
    {
        starpu_insert_task(
            ...
            STARPU_RW,        RTBLKADDR(U, MORSE_Complex32_t, Um, Un),
            STARPU_VALUE,    &ldu,                       sizeof(int),
            STARPU_RW,        RTBLKADDR(A, MORSE_Complex32_t, Am, An),
            STARPU_VALUE,    &lda,                       sizeof(int),
            STARPU_W,         RTBLKADDR(L, MORSE_Complex32_t, Lm, Ln),
...
static void cl_ctstrf_cpu_func(void *descr[], void *cl_arg)
{
    ...
    U = (MORSE_Complex32_t *)STARPU_MATRIX_GET_PTR(descr[0]);
    A = (MORSE_Complex32_t *)STARPU_MATRIX_GET_PTR(descr[1]);
    L = (MORSE_Complex32_t *)STARPU_MATRIX_GET_PTR(descr[2]);
    WORK = (MORSE_Complex32_t *)STARPU_MATRIX_GET_PTR(descr[3]);
    ...
    if ( ldl >= 2*ib )
    {
        ...
        for (i=0; i<n; i+=ib) {
            sb = min( ib, n-i );
            CORE_clacpy(MorseUpperLower, sb, sb, L+(i*ldl), ldl, L+(i*ldl)+ib, ldl );

            CORE_ctrtri( MorseLower, MorseUnit, sb, L+(i*ldl)+ib, ldl, &info );
            if (info != 0 ) {
...
static void cl_ctstrf_cuda_func(void *descr[], void *cl_arg)
{
    MORSE_starpu_ws_t *d_work;
    int m;
    int n;
    int ib;
    int nb;
    cuFloatComplex *hU, *dU;
    int ldu;
    cuFloatComplex *hA, *dA;
    int lda;
    cuFloatComplex *hL, *dL;
    int ldl;
    int *ipiv;
    cuFloatComplex *hw2, *hw, *dw;
    int ldwork;
    MORSE_bool check_info;
    int iinfo;
    int info;
    ...
    dU = (cuFloatComplex *)STARPU_MATRIX_GET_PTR(descr[0]);
    dA = (cuFloatComplex *)STARPU_MATRIX_GET_PTR(descr[1]);
    dL = (cuFloatComplex *)STARPU_MATRIX_GET_PTR(descr[2]);

    /*
     *  hwork => 2*nb*(2*ib+2nb)
     *  dwork => 2*ib*nb
     */
    hw2 = (cuFloatComplex *)STARPU_MATRIX_GET_PTR(descr[3]);
    dw  = (cuFloatComplex*)RUNTIME_starpu_ws_getlocal(d_work);
    ...
    cublasGetMatrix( nb, n,  sizeof(cuFloatComplex), dU, ldu, hU, ldu );
    cublasGetMatrix( m,  ib, sizeof(cuFloatComplex), dA, lda, hA, lda );

    /* Initialize L to 0 */
    memset(hL, 0, ldl*nb*sizeof(cuFloatComplex));

    magma_ctstrf_gpu( MagmaColMajor, m, n, ib, nb,
                      hU, ldu, dU, ldu,
                      hA, lda, dA, lda,
                      hL, ldl, dL, ldl,
                      ipiv,
...
CODELETS(ctstrf, 4, cl_ctstrf_cpu_func, cl_ctstrf_cuda_func, 0)
#else
CODELETS_CPU(ctstrf, 4, cl_ctstrf_cpu_func)
#endif
