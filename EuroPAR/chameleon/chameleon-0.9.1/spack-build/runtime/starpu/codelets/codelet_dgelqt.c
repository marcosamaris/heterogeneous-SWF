 ...
 *
 *  CORE_dgelqt - computes a LQ factorization of a real M-by-N tile A: A = L * Q.
 *
...
{
    (void)nb;
    struct starpu_codelet *codelet = &cl_dgelqt;
    void (*callback)(void*) = options->profiling ? cl_dgelqt_callback : NULL;
...
static void cl_dgelqt_cpu_func(void *descr[], void *cl_arg)
{
    ...
    TAU = (double *)STARPU_MATRIX_GET_PTR(descr[2]); /* max(m,n) + ib*n */
...
magma_int_t
magma_dgelqt_gpu( magma_int_t m, magma_int_t n, magma_int_t nb,
...
#define t_ref(a_1,a_2)  ( t+(a_2)*(ldt) + (a_1))
    ...
    memset(t_ref(0,0), 0, nb*n*sizeof(double));
    cudaMemset(dt_ref(0,0), 0, nb*n*sizeof(double));

    /* copy first panel of A on the host */
    cublasGetMatrix(min(m, nb), n, sizeof(double),
                    da_ref(0, 0), ldda,
                    v, ldv);
    ...
    for (i = 0; i < k; i += nb) {
        ...
        if (i > 0){
            ...
            cublasGetMatrix(ib, n, sizeof(double),
                            da_ref(i, 0), ldda,
                            v, ldv);
            ...
            if (rows > 0){
                magma_dlarfb_gpu( MagmaRight, MagmaNoTrans, MagmaForward, MagmaRowwise,
                                  rows, n-old_i, old_ib,
            ...
            magma_dgemerge_gpu(MagmaRight, MagmaUnit, old_ib, old_ib,
        ...
        CORE_dgelqt(ib, cols, ib,
                    (double*) v_ref(0,i), ldv,
        ...
        if ( i + ib < m ){
            ...
            CORE_dgesplit(MorseRight, MorseUnit, ib, min(cols,ib),
                          (double*) v_ref(0,i), ldv,
                          (double*) d, ldd);

            /* copy from host to device a tile diag */
            cublasSetMatrix( ib, min(cols,ib), sizeof(double),
                             d, ldd, dd, ldd );
        }
...
static void cl_dgelqt_cuda_func(void *descr[], void *cl_arg)
{
    MORSE_starpu_ws_t *h_work;
    int m;
    int n;
    ...
    d_W = (double *)STARPU_MATRIX_GET_PTR(descr[2]); /* m*ib + ib*ib*/
    d_D = d_W + m*ib;
...
CODELETS(dgelqt, 3, cl_dgelqt_cpu_func, cl_dgelqt_cuda_func, 0)
#else
CODELETS_CPU(dgelqt, 3, cl_dgelqt_cpu_func)
#endif
