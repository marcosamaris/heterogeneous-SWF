...
{
    struct starpu_codelet *codelet = &cl_cgessq;
...
static void cl_cgessq_cpu_func(void *descr[], void *cl_arg)
{
    int m;
    int n;
    MORSE_Complex32_t *A;
    ...
    CORE_cgessq( m, n, A, lda, &SCALESUMSQ[0], &SCALESUMSQ[1] );
}
/*
 * Codelet definition
 */
CODELETS_CPU(cgessq, 2, cl_cgessq_cpu_func)
