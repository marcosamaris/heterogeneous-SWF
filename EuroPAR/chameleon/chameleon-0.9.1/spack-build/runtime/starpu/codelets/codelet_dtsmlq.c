 ...
 *
 *  CORE_dtsmlq overwrites the general real M1-by-N1 tile A1 and
...
{
    struct starpu_codelet *codelet = &cl_dtsmlq;
    void (*callback)(void*) = options->profiling ? cl_dtsmlq_callback : NULL;
...
static void cl_dtsmlq_cpu_func(void *descr[], void *cl_arg)
{
    MORSE_enum side;
    MORSE_enum trans;
    int m1;
...
magma_int_t
magma_dtsmlq_gpu( magma_side_t side, magma_trans_t trans,
                  ...
                  CUstream stream)
{
    ...
    for(i = i1; (i > -1) && (i < K); i += i3) {
         ...
         */
        magma_dparfb_gpu( side, trans, MagmaForward, MagmaRowwise,
                          mi, ni, M2, N2, kb, 0,
                          A1 + LDA1*jc+ic, LDA1,
                          A2, LDA2,
                          V + i, LDV,
...
static void cl_dtsmlq_cuda_func(void *descr[], void *cl_arg)
{
    MORSE_enum side;
    MORSE_enum trans;
    int m1;
    ...
    magma_dtsmlq_gpu( side, trans, m1, n1, m2, n2, k, ib,
...
CODELETS(dtsmlq, 5, cl_dtsmlq_cpu_func, cl_dtsmlq_cuda_func, STARPU_CUDA_ASYNC)
#else
CODELETS_CPU(dtsmlq, 5, cl_dtsmlq_cpu_func)
#endif
