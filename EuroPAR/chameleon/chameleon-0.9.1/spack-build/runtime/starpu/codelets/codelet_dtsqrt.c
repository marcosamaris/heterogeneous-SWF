...
{
    (void)nb;
    struct starpu_codelet *codelet = &cl_dtsqrt;
    void (*callback)(void*) = options->profiling ? cl_dtsqrt_callback : NULL;
...
static void cl_dtsqrt_cpu_func(void *descr[], void *cl_arg)
{
...
magma_int_t
magma_dtsqrt2_gpu( magma_int_t m, magma_int_t n, magma_int_t nb,
...
//	cublasGetMatrix(m, nb, sizeof(double),
//                    da2_ref(0, 0), ldda2,
//                    a2, lda2);
...
//                rows*ib*sizeof(double),
//                cudaMemcpyDeviceToHost );

			/* Apply H' to A(i:m,i+2*ib:n) from the left */
			cols = n-old_i-2*old_ib;
			if (cols > 0){
				magma_dtsmqr_gpu( MagmaLeft, MagmaTrans,
...
static void cl_dtsqrt_cuda_func(void *descr[], void *cl_arg)
{
    MORSE_starpu_ws_t *h_work;
    int m;
    int n;
    ...
    d_W  = (double *)STARPU_MATRIX_GET_PTR(descr[3]); /* 2*ib*n + ib*ib */
    d_D  = d_W + 2*ib*n;
...
CODELETS(dtsqrt, 4, cl_dtsqrt_cpu_func, cl_dtsqrt_cuda_func, 0)
#else
CODELETS_CPU(dtsqrt, 4, cl_dtsqrt_cpu_func)
#endif
