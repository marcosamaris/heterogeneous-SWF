...
#define _NAME  "MORSE_clange_Tile"
...
RunTest(int *iparam, float *dparam, morse_time_t *t_)
{
    ...
    MORSE_cplrnt_Tile( descA, 3436 );

    /* MORSE CPOSV */
    START_TIMING();
    normmorse = MORSE_clange_Tile(MorseInfNorm, descA);
    STOP_TIMING();
    ...
    {
        /* Allocate Data */
        PASTE_TILE_TO_LAPACK( descA, A, check, MORSE_Complex32_t, M, N );
        float *work = (float*) malloc(max(M,N)*sizeof(float));
        normlapack = LAPACKE_clange_work(LAPACK_COL_MAJOR, morse_lapack_const(MorseInfNorm), M, N, A, LDA, work);
