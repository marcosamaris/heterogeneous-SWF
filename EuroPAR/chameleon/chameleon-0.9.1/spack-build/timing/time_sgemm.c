...
RunTest(int *iparam, float *dparam, morse_time_t *t_) 
{
    ...
    MORSE_splrnt( M, K, A, LDA,  453 );
    MORSE_splrnt( K, N, B, LDB, 5673 );
    MORSE_splrnt( M, N, C, LDC,  740 );
    ...
    START_TIMING();
    MORSE_sgemm( MorseNoTrans, MorseNoTrans, M, N, K, alpha, A, LDA, B, LDB, beta, C, LDC );
    STOP_TIMING();
