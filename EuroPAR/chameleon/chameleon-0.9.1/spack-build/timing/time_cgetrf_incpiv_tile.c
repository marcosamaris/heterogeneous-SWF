...
RunTest(int *iparam, float *dparam, morse_time_t *t_) 
{
    ...
    PASTE_CODE_ALLOCATE_MATRIX_TILE( descX,  check, MORSE_Complex32_t, MorseComplexFloat, LDB, M, NRHS );
    PASTE_CODE_ALLOCATE_MATRIX_TILE( descAC, check, MORSE_Complex32_t, MorseComplexFloat, LDA, M, N    );
    PASTE_CODE_ALLOCATE_MATRIX_TILE( descB,  check, MORSE_Complex32_t, MorseComplexFloat, LDB, M, NRHS );
    
    MORSE_cplrnt_Tile(descA, 3456);

    /* Allocate Workspace */
    MORSE_Alloc_Workspace_cgesv_incpiv_Tile(min(M,N), &descL, &piv);
    ...
    START_TIMING();
    MORSE_cgetrf_incpiv_Tile( descA, descL, piv );
    STOP_TIMING();
    ...
    if ( check )
    {
        ...
        MORSE_cgetrs_incpiv_Tile( descA, descL, piv, descX );
