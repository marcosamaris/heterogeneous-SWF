...
#define _NAME  "MORSE_cpotri_Tile"
...
RunTest(int *iparam, float *dparam, morse_time_t *t_) 
{
...
#if defined(TRACE_BY_SEQUENCE)
    {
        ...
        if ( ! iparam[IPARAM_ASYNC] ) {
            ...
            MORSE_cpotrf_Tile_Async(uplo, descA,                sequence[0], &request[0]);
            MORSE_Sequence_Wait(sequence[0]);

            MORSE_ctrtri_Tile_Async(uplo, MorseNonUnit, descA, sequence[1], &request[1]);
            MORSE_Sequence_Wait(sequence[1]);

            MORSE_clauum_Tile_Async(uplo, descA,                sequence[2], &request[2]);
...
#else
    {
        if ( ! iparam[IPARAM_ASYNC] ) {
            ...
            MORSE_ctrtri_Tile(uplo, MorseNonUnit, descA);
            MORSE_clauum_Tile(uplo, descA);
            STOP_TIMING();
            ...
            MORSE_cpotrf_Tile_Async(uplo, descA, sequence, &request[0]);
            MORSE_cpotri_Tile_Async(uplo, descA, sequence, &request[1]);
