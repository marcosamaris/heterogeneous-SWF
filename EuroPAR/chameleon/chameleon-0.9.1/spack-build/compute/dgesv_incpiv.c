 ...
 * @ingroup double
 *
 *  MORSE_dgesv_incpiv - Computes the solution to a system of linear equations A * X = B,
 ...
 *
 ******************************************************************************/
int MORSE_dgesv_incpiv(int N, int NRHS,
                 double *A, int LDA,
                 ...
                 double *B, int LDB)
{
    ...
    if (morse == NULL) {
        morse_error("MORSE_dgesv_incpiv", "MORSE not initialized");
        return MORSE_ERR_NOT_INITIALIZED;
    }
    /* Check input arguments */
    if (N < 0) {
        morse_error("MORSE_dgesv_incpiv", "illegal value of N");
        return -1;
    }
    if (NRHS < 0) {
        morse_error("MORSE_dgesv_incpiv", "illegal value of NRHS");
        return -2;
    }
    if (LDA < max(1, N)) {
        morse_error("MORSE_dgesv_incpiv", "illegal value of LDA");
        return -4;
    }
    if (LDB < max(1, N)) {
        morse_error("MORSE_dgesv_incpiv", "illegal value of LDB");
        return -8;
    }
    /* Quick return */
    if (min(N, NRHS) == 0)
    ...
    if (status != MORSE_SUCCESS) {
        morse_error("MORSE_dgesv_incpiv", "morse_tune() failed");
        return status;
    }
...
/*    }*/
    ...
    MORSE_dgesv_incpiv_Tile_Async(&descA, descL, IPIV, &descB, sequence, &request);
 ...
 * @ingroup double_Tile
 *
 *  MORSE_dgesv_incpiv_Tile - Solves a system of linear equations using the tile LU factorization.
 *  Tile equivalent of MORSE_dgetrf_incpiv().
...
int MORSE_dgesv_incpiv_Tile(MORSE_desc_t *A, MORSE_desc_t *L, int *IPIV, MORSE_desc_t *B)
{
    ...
    morse = morse_context_self();
    if (morse == NULL) {
        morse_fatal_error("MORSE_dgesv_incpiv_Tile", "MORSE not initialized");
        return MORSE_ERR_NOT_INITIALIZED;
    }
    morse_sequence_create(morse, &sequence);
    MORSE_dgesv_incpiv_Tile_Async(A, L, IPIV, B, sequence, &request);
    RUNTIME_barrier(morse);
 ...
 *
 *  MORSE_dgesv_incpiv_Tile_Async - Solves a system of linear equations using the tile
 *  LU factorization.
 *  Non-blocking equivalent of MORSE_dgesv_incpiv_Tile().
 ...
 * @sa MORSE_dgesv_incpiv_Tile_Async
 * @sa MORSE_sgesv_incpiv_Tile_Async
 * @sa MORSE_dsgesv_Tile_Async
 *
 ******************************************************************************/
int MORSE_dgesv_incpiv_Tile_Async(MORSE_desc_t *A, MORSE_desc_t *L, int *IPIV, MORSE_desc_t *B,
                            MORSE_sequence_t *sequence, MORSE_request_t *request)
{
    ...
    if (sequence == NULL) {
        morse_fatal_error("MORSE_dgesv_incpiv_Tile", "NULL sequence");
        return MORSE_ERR_UNALLOCATED;
    }
    if (request == NULL) {
        morse_fatal_error("MORSE_dgesv_incpiv_Tile", "NULL request");
        return MORSE_ERR_UNALLOCATED;
    }
    ...
    if (morse_desc_check(A) != MORSE_SUCCESS) {
        morse_error("MORSE_dgesv_incpiv_Tile", "invalid first descriptor");
        return morse_request_fail(sequence, request, MORSE_ERR_ILLEGAL_VALUE);
    }
    if (morse_desc_check(L) != MORSE_SUCCESS) {
        morse_error("MORSE_dgesv_incpiv_Tile", "invalid second descriptor");
        return morse_request_fail(sequence, request, MORSE_ERR_ILLEGAL_VALUE);
    }
    if (morse_desc_check(B) != MORSE_SUCCESS) {
        morse_error("MORSE_dgesv_incpiv_Tile", "invalid third descriptor");
        return morse_request_fail(sequence, request, MORSE_ERR_ILLEGAL_VALUE);
    }
    /* Check input arguments */
    if (A->nb != A->mb || B->nb != B->mb) {
        morse_error("MORSE_dgesv_incpiv_Tile", "only square tiles supported");
        return morse_request_fail(sequence, request, MORSE_ERR_ILLEGAL_VALUE);
    }
    /* Quick return */
/*
...
*/
    ...
    morse_pdgetrf_incpiv(A, L, IPIV, sequence, request);
