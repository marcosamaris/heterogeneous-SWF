 ...
 *
 ******************************************************************************/
int MORSE_dpotrf(MORSE_enum uplo, int N,
                  double *A, int LDA)
{
    int NB;
    int status;
    ...
    if (morse == NULL) {
        morse_fatal_error("MORSE_dpotrf", "MORSE not initialized");
        return MORSE_ERR_NOT_INITIALIZED;
    }
    /* Check input arguments */
    if (uplo != MorseUpper && uplo != MorseLower) {
        morse_error("MORSE_dpotrf", "illegal value of uplo");
        return -1;
    }
    if (N < 0) {
        morse_error("MORSE_dpotrf", "illegal value of N");
        return -2;
    }
    if (LDA < max(1, N)) {
        morse_error("MORSE_dpotrf", "illegal value of LDA");
        return -4;
    }
    /* Quick return */
    if (max(N, 0) == 0)
    ...
    if (status != MORSE_SUCCESS) {
        morse_error("MORSE_dpotrf", "morse_tune() failed");
        return status;
    }
 ...
 *
 ******************************************************************************/
int MORSE_dpotrf_Tile_Async(MORSE_enum uplo, MORSE_desc_t *A,
