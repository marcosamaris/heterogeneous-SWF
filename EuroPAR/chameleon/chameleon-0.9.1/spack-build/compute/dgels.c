 ...
 * @ingroup double
 *
 *  MORSE_dgels - solves overdetermined or underdetermined linear systems involving an M-by-N
 ...
 *
 ******************************************************************************/
int MORSE_dgels(MORSE_enum trans, int M, int N, int NRHS,
                 double *A, int LDA,
                 MORSE_desc_t *descT,
                 double *B, int LDB)
{
    ...
    if (morse == NULL) {
        morse_fatal_error("MORSE_dgels", "MORSE not initialized");
        return MORSE_ERR_NOT_INITIALIZED;
    }

    /* Check input arguments */
    if (trans != MorseNoTrans) {
        morse_error("MORSE_dgels", "only MorseNoTrans supported");
        return MORSE_ERR_NOT_SUPPORTED;
    }
    if (M < 0) {
        morse_error("MORSE_dgels", "illegal value of M");
        return -2;
    }
    if (N < 0) {
        morse_error("MORSE_dgels", "illegal value of N");
        return -3;
    }
    if (NRHS < 0) {
        morse_error("MORSE_dgels", "illegal value of NRHS");
        return -4;
    }
    if (LDA < max(1, M)) {
        morse_error("MORSE_dgels", "illegal value of LDA");
        return -6;
    }
    if (LDB < max(1, max(M, N))) {
        morse_error("MORSE_dgels", "illegal value of LDB");
        return -9;
    }
    /* Quick return */
    ...
    status = morse_tune(MORSE_FUNC_DGELS, M, N, NRHS);
    if (status != MORSE_SUCCESS) {
        morse_error("MORSE_dgels", "morse_tune() failed");
        return status;
    }
...
/*        if ( MORSE_TRANSLATION == MORSE_OUTOFPLACE ) {*/
            ...
            morse_dooplap2tile( descB, B, NB, NB, LDB, NRHS, 0, 0, M, NRHS, sequence, &request,
                                 morse_desc_mat_free(&(descA)); morse_desc_mat_free(&(descB)));
/*        } else {*/
/*            morse_diplap2tile( descA, A, NB, NB, LDA, N,    0, 0, M, N,*/
/*                                sequence, &request);*/
/*            morse_diplap2tile( descB, B, NB, NB, LDB, NRHS, 0, 0, M, NRHS,*/
/*                                sequence, &request);*/
/*        }*/
    } else {
/*        if ( MORSE_TRANSLATION == MORSE_OUTOFPLACE ) {*/
...
/*        }*/
    ...
    MORSE_dgels_Tile_Async(MorseNoTrans, &descA, descT, &descB, sequence, &request);
 ...
 * @ingroup double_Tile
 *
 *  MORSE_dgels_Tile - Solves overdetermined or underdetermined linear system of equations
 *  using the tile QR or the tile LQ factorization.
 *  Tile equivalent of MORSE_dgels().
 ...
 *
 ******************************************************************************/
int MORSE_dgels_Tile(MORSE_enum trans, MORSE_desc_t *A,
                      MORSE_desc_t *T, MORSE_desc_t *B)
{
    ...
    morse = morse_context_self();
    if (morse == NULL) {
        morse_fatal_error("MORSE_dgels_Tile", "MORSE not initialized");
        return MORSE_ERR_NOT_INITIALIZED;
    }
    morse_sequence_create(morse, &sequence);
    MORSE_dgels_Tile_Async(trans, A, T, B, sequence, &request);
    RUNTIME_barrier(morse);
 ...
 *
 *  MORSE_dgels_Tile_Async - Solves overdetermined or underdetermined linear
 *  system of equations using the tile QR or the tile LQ factorization.
 *  Non-blocking equivalent of MORSE_dgels_Tile().
 ...
 *
 ******************************************************************************/
int MORSE_dgels_Tile_Async(MORSE_enum trans, MORSE_desc_t *A,
                           MORSE_desc_t *T, MORSE_desc_t *B,
                           MORSE_sequence_t *sequence, MORSE_request_t *request)
{
    ...
    if (sequence == NULL) {
        morse_fatal_error("MORSE_dgels_Tile", "NULL sequence");
        return MORSE_ERR_UNALLOCATED;
    }
    if (request == NULL) {
        morse_fatal_error("MORSE_dgels_Tile", "NULL request");
        return MORSE_ERR_UNALLOCATED;
    }
    ...
    if (morse_desc_check(A) != MORSE_SUCCESS) {
        morse_error("MORSE_dgels_Tile", "invalid first descriptor");
        return morse_request_fail(sequence, request, MORSE_ERR_ILLEGAL_VALUE);
    }
    if (morse_desc_check(T) != MORSE_SUCCESS) {
        morse_error("MORSE_dgels_Tile", "invalid second descriptor");
        return morse_request_fail(sequence, request, MORSE_ERR_ILLEGAL_VALUE);
    }
    if (morse_desc_check(B) != MORSE_SUCCESS) {
        morse_error("MORSE_dgels_Tile", "invalid third descriptor");
        return morse_request_fail(sequence, request, MORSE_ERR_ILLEGAL_VALUE);
    }
    /* Check input arguments */
    if (A->nb != A->mb || B->nb != B->mb) {
        morse_error("MORSE_dgels_Tile", "only square tiles supported");
        return morse_request_fail(sequence, request, MORSE_ERR_ILLEGAL_VALUE);
    }
    if (trans != MorseNoTrans) {
        morse_error("MORSE_dgels_Tile", "only MorseNoTrans supported");
    ...
    if (A->m >= A->n) {
        if (morse->householder == MORSE_FLAT_HOUSEHOLDER) {
            morse_pdgeqrf(A, T, sequence, request);

            morse_pdormqr(MorseLeft, MorseTrans, A, B, T, sequence, request);
        }
        else {
            morse_pdgeqrfrh(A, T, MORSE_RHBLK, sequence, request);

            morse_pdormqrrh(MorseLeft, MorseTrans, A, B, T, MORSE_RHBLK, sequence, request);
        }
        subB = morse_desc_submatrix(B, 0, 0, A->n, B->n);
        subA = morse_desc_submatrix(A, 0, 0, A->n, A->n);
        morse_pdtrsm(MorseLeft, MorseUpper, MorseNoTrans, MorseNonUnit, 1.0, subA, subB, sequence, request);
        free(subA);
        free(subB);
    }
    else {
        /* subB = morse_desc_submatrix(B, A->m, 0, A->n-A->m, B->n);
        morse_pdtile_zero(subB, sequence, request);
        free(subB); */
        ...
        morse_pdtrsm(MorseLeft, MorseLower, MorseNoTrans, MorseNonUnit, 1.0, subA, subB, sequence, request);
        free(subA);
        free(subB);

        if (morse->householder == MORSE_FLAT_HOUSEHOLDER) {
            morse_pdormlq(MorseLeft, MorseTrans, A, B, T, sequence, request);
        }
        else {
            morse_pdormlqrh(MorseLeft, MorseTrans, A, B, T, MORSE_RHBLK, sequence, request);
        }
    }
    return MORSE_SUCCESS;
}
