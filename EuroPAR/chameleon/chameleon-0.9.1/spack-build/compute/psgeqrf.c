 ...
 **/
void morse_psgeqrf(MORSE_desc_t *A, MORSE_desc_t *T,
                   MORSE_sequence_t *sequence, MORSE_request_t *request)
{
    ...
    /* Worker space
     *
     * sgeqrt = max( A->nb * (ib+1), ib * (ib + A->nb) )
     * sormqr = A->nb * ib
     * stsqrt = max( A->nb * (ib+1), ib * (ib + A->nb) )
     * stsmqr = 2 * A->nb * ib
     */
    ...
    /* Host space
     *
     * sgeqrt = ib * (A->mb+3*ib) + A->mb )
     * stsqrt = 2 * ib * (A->nb+ib) + A->nb
     */
    ...
    /* necessary to avoid dependencies between tsqrt and ormqr tasks regarding the diag tile */
    DIAG = (MORSE_desc_t*)malloc(sizeof(MORSE_desc_t));
    morse_sdesc_alloc2(*DIAG, A->mb, A->nb, min(A->m, A->n), A->nb, 0, 0, min(A->m, A->n), A->nb);
