 ...
 **/
void morse_pdgeqrfrh(MORSE_desc_t *A, MORSE_desc_t *T, int BS,
                     MORSE_sequence_t *sequence, MORSE_request_t *request)
{
    ...
    /* Worker space
     *
     * dgeqrt = max( A->nb * (ib+1), ib * (ib + A->nb) )
     * dormqr = A->nb * ib
     * dtsqrt = max( A->nb * (ib+1), ib * (ib + A->nb) )
     * dtsmqr = 2 * A->nb * ib
     */
    ...
    /* Host space
     *
     * dgeqrt = ib * (A->nb+3*ib) + A->nb )
     * dtsqrt = 2 * ib * (A->nb+ib) + A->nb
     */
    ...
    morse_ddesc_alloc2(*DIAG, A->mb, A->nb, nblk * A->mb, A->nb, 0, 0, nblk * A->mb, A->nb);
