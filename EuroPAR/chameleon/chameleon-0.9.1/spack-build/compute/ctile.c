 ...
 *
 *  MORSE_cLapack_to_Tile - Conversion from LAPACK layout to tile layout.
 *
 *******************************************************************************
 *
 * @param[in] Af77
...
int MORSE_cLapack_to_Tile(MORSE_Complex32_t *Af77, int LDA, MORSE_desc_t *A)
{
    ...
    if (morse == NULL) {
        morse_fatal_error("MORSE_cLapack_to_Tile", "MORSE not initialized");
        return MORSE_ERR_NOT_INITIALIZED;
    }
    /* Check descriptor for correctness */
    if (morse_desc_check( A ) != MORSE_SUCCESS) {
        morse_error("MORSE_cLapack_to_Tile", "invalid descriptor");
 ...
 *
 *  MORSE_cLapack_to_Tile_Async - Conversion from LAPACK layout to tile layout.
 *  Non-blocking equivalent of MORSE_cLapack_to_Tile().
 ...
 *
 ******************************************************************************/
int MORSE_cLapack_to_Tile_Async(MORSE_Complex32_t *Af77, int LDA, MORSE_desc_t *A,
...
int MORSE_cTile_to_Lapack(MORSE_desc_t *A, MORSE_Complex32_t *Af77, int LDA)
{
    ...
    morse = morse_context_self();
    if (morse == NULL) {
        morse_fatal_error("MORSE_cTile_to_Lapack", "MORSE not initialized");
        return MORSE_ERR_NOT_INITIALIZED;
    }
    /* Check descriptor for correctness */
    if (morse_desc_check( A ) != MORSE_SUCCESS) {
        morse_error("MORSE_cTile_to_Lapack", "invalid descriptor");
        return MORSE_ERR_ILLEGAL_VALUE;
    }
    morse_sequence_create(morse, &sequence);
 ...
 * @ingroup MORSE_Complex32_t_Tile_Async
 *
 *  MORSE_cTile_to_Lapack_Async - Conversion from LAPACK layout to tile layout.
 *  Non-blocking equivalent of MORSE_cTile_to_Lapack().
 *  May return before the computation is finished.
 *  Allows for pipelining of operations ar runtime.
 *
 *
 ...
 *
 ******************************************************************************/
int MORSE_cTile_to_Lapack_Async(MORSE_desc_t *A, MORSE_Complex32_t *Af77, int LDA,
                                MORSE_sequence_t *sequence, MORSE_request_t *request)
{
    MORSE_context_t *morse;
