 ...
 *
 *  MORSE_cgetrf_incpiv - Computes an LU factorization of a general M-by-N matrix A
 ...
 *          required by MORSE_cgetrs_incpiv to solve the system of equations.
 *
 * @param[out] IPIV
 ...
 *
 ******************************************************************************/
int MORSE_cgetrf_incpiv(int M, int N,
                  ...
                  MORSE_desc_t *descL, int *IPIV)
{
    ...
    if (morse == NULL) {
        morse_fatal_error("MORSE_cgetrf_incpiv", "MORSE not initialized");
        return MORSE_ERR_NOT_INITIALIZED;
    }
    /* Check input arguments */
    if (M < 0) {
        morse_error("MORSE_cgetrf_incpiv", "illegal value of M");
        return -1;
    }
    if (N < 0) {
        morse_error("MORSE_cgetrf_incpiv", "illegal value of N");
        return -2;
    }
    if (LDA < max(1, M)) {
        morse_error("MORSE_cgetrf_incpiv", "illegal value of LDA");
        return -4;
    }
    /* Quick return */
    if (min(M, N) == 0)
    ...
    if (status != MORSE_SUCCESS) {
        morse_error("MORSE_cgetrf_incpiv", "morse_tune() failed");
        return status;
    }
...
/*    }*/
    ...
    MORSE_cgetrf_incpiv_Tile_Async(&descA, descL, IPIV, sequence, &request);
 ...
 *
 *  MORSE_cgetrf_incpiv_Tile - Computes the tile LU factorization of a matrix.
...
int MORSE_cgetrf_incpiv_Tile(MORSE_desc_t *A, MORSE_desc_t *L, int *IPIV)
{
    ...
    morse = morse_context_self();
    if (morse == NULL) {
        morse_fatal_error("MORSE_cgetrf_incpiv_Tile", "MORSE not initialized");
        return MORSE_ERR_NOT_INITIALIZED;
    }
    morse_sequence_create(morse, &sequence);
    MORSE_cgetrf_incpiv_Tile_Async(A, L, IPIV, sequence, &request);
    RUNTIME_barrier(morse);
 ...
 *
 *  MORSE_cgetrf_incpiv_Tile_Async - Computes the tile LU factorization of a matrix.
 *  Non-blocking equivalent of MORSE_cgetrf_incpiv_Tile().
 ...
 *
 ******************************************************************************/
int MORSE_cgetrf_incpiv_Tile_Async(MORSE_desc_t *A, MORSE_desc_t *L, int *IPIV,
                             MORSE_sequence_t *sequence, MORSE_request_t *request)
{
    ...
    if (sequence == NULL) {
        morse_fatal_error("MORSE_cgetrf_incpiv_Tile", "NULL sequence");
        return MORSE_ERR_UNALLOCATED;
    }
    if (request == NULL) {
        morse_fatal_error("MORSE_cgetrf_incpiv_Tile", "NULL request");
        return MORSE_ERR_UNALLOCATED;
    }
    ...
    if (morse_desc_check(A) != MORSE_SUCCESS) {
        morse_error("MORSE_cgetrf_incpiv_Tile", "invalid first descriptor");
        return morse_request_fail(sequence, request, MORSE_ERR_ILLEGAL_VALUE);
    }
    if (morse_desc_check(L) != MORSE_SUCCESS) {
        morse_error("MORSE_cgetrf_incpiv_Tile", "invalid second descriptor");
        return morse_request_fail(sequence, request, MORSE_ERR_ILLEGAL_VALUE);
    }
    /* Check input arguments */
    if (A->nb != A->mb) {
        morse_error("MORSE_cgetrf_incpiv_Tile", "only square tiles supported");
        return morse_request_fail(sequence, request, MORSE_ERR_ILLEGAL_VALUE);
    }
    /* Quick return */
/*
