 ...
 * @ingroup double
 *
 *  MORSE_dsyr2k - Performs one of the symmetric rank 2k operations
 *
 ...
 *******************************************************************************
 *
 * @sa MORSE_dsyr2k_Tile
 * @sa MORSE_csyr2k
 * @sa MORSE_dsyr2k
 * @sa MORSE_ssyr2k
 *
 ******************************************************************************/
int MORSE_dsyr2k(MORSE_enum uplo, MORSE_enum trans, int N, int K,
                  ...
                  double beta,  double *C, int LDC)
{
    ...
    if (morse == NULL) {
        morse_fatal_error("MORSE_dsyr2k", "MORSE not initialized");
        return MORSE_ERR_NOT_INITIALIZED;
    }

    /* Check input arguments */
    if ((uplo != MorseUpper) && (uplo != MorseLower)) {
        morse_error("MORSE_dsyr2k", "illegal value of uplo");
        return -1;
    }
    if ((trans != MorseNoTrans) && (trans != MorseTrans)) {
        morse_error("MORSE_dsyr2k", "illegal value of trans");
        return -2;
    }
    ...
    if (N < 0) {
        morse_error("MORSE_dsyr2k", "illegal value of N");
        return -3;
    }
    if (K < 0) {
        morse_error("MORSE_dsyr2k", "illegal value of K");
        return -4;
    }
    if (LDA < max(1, Am)) {
        morse_error("MORSE_dsyr2k", "illegal value of LDA");
        return -7;
    }
    if (LDB < max(1, Am)) {
        morse_error("MORSE_dsyr2k", "illegal value of LDB");
        return -9;
    }
    if (LDC < max(1, N)) {
        morse_error("MORSE_dsyr2k", "illegal value of LDC");
        return -12;
    }
    ...
    if (status != MORSE_SUCCESS) {
        morse_error("MORSE_dsyr2k", "morse_tune() failed");
        return status;
    }
...
/*    if ( MORSE_TRANSLATION == MORSE_OUTOFPLACE ) {*/
        ...
        morse_dooplap2tile( descB, B, NB, NB, LDB, An, 0, 0, Am, An, sequence, &request,
...
/*                            sequence, &request);*/
/*        morse_diplap2tile( descB, B, NB, NB, LDB, An, 0, 0, Am, An, */
/*                            sequence, &request);*/
...
/*    }*/
    ...
    MORSE_dsyr2k_Tile_Async(uplo, trans, alpha, &descA, &descB, beta, &descC, sequence, &request);
...
/*        morse_diptile2lap( descB, B, NB, NB, LDB, An,  sequence, &request);*/
 ...
 * @ingroup double_Tile
 *
 *  MORSE_dsyr2k_Tile - Performs symmetric rank k update.
 *  Tile equivalent of MORSE_dsyr2k().
 ...
 *
 ******************************************************************************/
int MORSE_dsyr2k_Tile(MORSE_enum uplo, MORSE_enum trans,
                       ...
                       double beta,  MORSE_desc_t *C)
{
    ...
    morse = morse_context_self();
    if (morse == NULL) {
        morse_fatal_error("MORSE_dsyr2k_Tile", "MORSE not initialized");
        return MORSE_ERR_NOT_INITIALIZED;
    }
    morse_sequence_create(morse, &sequence);
 ...
 *
 *  MORSE_dsyr2k_Tile_Async - Performs symmetric rank-k update.
 *  Non-blocking equivalent of MORSE_dsyr2k_Tile().
 ...
 *
 ******************************************************************************/
int MORSE_dsyr2k_Tile_Async(MORSE_enum uplo, MORSE_enum trans,
                             double alpha, MORSE_desc_t *A, MORSE_desc_t *B,
                             double beta,  MORSE_desc_t *C,
                             MORSE_sequence_t *sequence, MORSE_request_t *request)
{
    ...
    morse = morse_context_self();
    if (morse == NULL) {
        morse_fatal_error("MORSE_dsyr2k_Tile_Async", "MORSE not initialized");
        return MORSE_ERR_NOT_INITIALIZED;
    }
    if (sequence == NULL) {
        morse_fatal_error("MORSE_dsyr2k_Tile_Async", "NULL sequence");
        return MORSE_ERR_UNALLOCATED;
    }
    if (request == NULL) {
        morse_fatal_error("MORSE_dsyr2k_Tile_Async", "NULL request");
        return MORSE_ERR_UNALLOCATED;
    }
    ...
    if (morse_desc_check(A) != MORSE_SUCCESS) {
        morse_error("MORSE_dsyr2k_Tile_Async", "invalid first descriptor");
        return morse_request_fail(sequence, request, MORSE_ERR_ILLEGAL_VALUE);
    }
    if (morse_desc_check(B) != MORSE_SUCCESS) {
        morse_error("MORSE_dsyr2k_Tile_Async", "invalid second descriptor");
        return morse_request_fail(sequence, request, MORSE_ERR_ILLEGAL_VALUE);
    }
    if (morse_desc_check(C) != MORSE_SUCCESS) {
        morse_error("MORSE_dsyr2k_Tile_Async", "invalid third descriptor");
        return morse_request_fail(sequence, request, MORSE_ERR_ILLEGAL_VALUE);
    }
    /* Check input arguments */
    if ((uplo != MorseUpper) && (uplo != MorseLower)) {
    ...
    if (C->mb != C->nb) {
        morse_error("MORSE_dsyr2k_Tile_Async", "only square tiles for C are supported");
        return morse_request_fail(sequence, request, MORSE_ERR_ILLEGAL_VALUE);
    }
    if ( (B->mb != A->mb) || (B->nb != A->nb) || (Amb != C->mb) ){
        morse_error("MORSE_dsyr2k_Tile_Async", "tile sizes have to match");
        return morse_request_fail(sequence, request, MORSE_ERR_ILLEGAL_VALUE);
    }
    if (C->m != C->n) {
        morse_error("MORSE_dsyr2k_Tile_Async", "only square matrix C is supported");
        return morse_request_fail(sequence, request, MORSE_ERR_ILLEGAL_VALUE);
    }
    if ( (B->m != A->m) || (B->n != A->n) || (Am != C->m) ){
        morse_error("MORSE_dsyr2k_Tile_Async", "sizes of matrices have to match");
        return morse_request_fail(sequence, request, MORSE_ERR_ILLEGAL_VALUE);
    }
    ...
    morse_pdsyr2k(uplo, trans, alpha, A, B, beta, C, sequence, request);
